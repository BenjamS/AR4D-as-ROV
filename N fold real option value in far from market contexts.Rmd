...
As mentioned above, AR4D projects are characterized by high, potentially disruptive, expected net benefits, but also by extreme uncertainty surrounding these expected net benefits. Expected project impacts are vulnerable to a host of both research and non-research related factors (to be discussed in more detail farther below), such that they can easily double or halve over the life of the project. Project appraisals that do not price in, or at least provide some treatment of, this vast uncertainty are thus of limited use to prospective donors.
<!-- given new additions to or subtractions from the list of countries targeted for release of the new technology. -->

One of the appealing advantages of the ROV approach, then, is precisely that it prices in project volatility. However, there remains the question of how to quantify vaolatility. In the financial context, a direct calculation of volatility is possible based on easily accessible historical price time series. In the real options context, direct calculation is generally not possible, since the analogous historical project NPV time series data usually do not exist.

In practice, applied ROV studies generally make an educated guess at the volatility parameter, and then conduct sensitivity analysis around the guess (as, for example, in Majd and Pindyck [-@majd1987time] or Kemna [-@kemna1993case]). This is expedient, but lacking in rigor, as it takes time to develop intuition about plausible project volatility parameter values. In a rare, novel approach, Pennings and Lint [-@pennings1997option] directly calculate the uncertainty parameter based on a painstakingly assembled project NPV time series. While clearly more rigorous than an educated guess, this is less expedient and probably not possible under most AR4D time, resource, and data constraints.

As an intermediate point on the rigor-expedience spectrum, consider that research managers (and/or stakeholders and/or the foresight economists conducting ex-ante impact assessments) generally have some idea of the uncertainty surrounding project NPV estimates, such that they can be asked to quantify minimum and maximum bounds on project NPV, in addition to the NPV estimation itself. That is, they can say that NPV is $x(\hat{t})$, but that it might be as much as $100 \%$ greater than $x(\hat{t})$ or as little as $50\%$ lower than $x(\hat{t})$.

Interpreting these upper and lower percentage errors as a $95\%$ confidence interval, both the NPV volatility and the mean NPV growth rate can usually be deduced from this information. If $x(t)$ is a gBm, for example, then the upper and lower percentage errors, $\overline{y}$, $\underline{y}$, may be expressed as follows.
...
&= E \left[ \ln \left( \frac{x(T)}{x(\hat{t})} \right) \right] \approx E \left[ \frac{\Delta x}{x(\hat{t})} \right] \\
\sigma &= s^2 \tau
\end{split}
\end{equation}

This system of equations may then be solved for the project volatility parameter $s$ and mean growth rate $m$ in terms of the percentage errors, the project time horizon $T$, and $z$, as follows.
...
2) A basic replication or scaling up stage, whereby a technology proof-of-concept generated in the discovery stage---typically in the form of a genotype with a novel trait, together with a set of recommended farm management practices---is reproduced in greenhouse and/or confined field trials.

3) A multi-location, multi-season testing stage, whereby successful specimens generated in the preceding stage are taken for further scaling up and testing under distinct agronomic conditions over multiple cropping seasons. [[Marc - mention marker assisted]]
...
Traditionally, stages 2-5 constitute a single AR4D project. Donors tend to engage in stage 1 through separate program level arrangements, since investments there are cross-cutting, i.e. they apply across multiple prospective technologies. Investments in stage 6 are also cross-cutting to the extent that they involve support of extension services, seed systems, and other value chain mechanisms. Stage 6 may involve some additional greenhouse and CFT work to adapt the new technology to the specific country of release. Details may vary widely depending on whether the new technology is released as a public good or under private contract.
...
To formalize the $n$-fold real option value of AR4D projects, let the real option value of a single stage project (equation \ref{eq:rovBraw}) evaluated at some time $\hat{t}$ henceforth be relabeled $f_1(\hat{t})$, with time horizon $T_1$, exercise cost $K_1$, and abandonment value $B_1$.
...
Below I apply the $n$-fold option value model derived above to valuate a real AR4D project currently in implementation. The aim of the project is to develop a transgenic potato variety with resistance to Late Blight disease (henceforth LBr potato), for release as a public good in one or more developing countries. Late Blight is the disease behind the infamous Irish Potato Famine of 1845-1849; and continues to pose a major threat to food security, especially in the developing world [@]. Successful research and development of LBr potato varieties adapted to local environments is thus a high reward, disruptive proposition. It is also a high risk proposition, facing numerous research and non-research related challenges.

The LBr potato project follows the 4 stage structure described in section \ref{sec:...}. The cost and time duration of each stage are summarized in Table \ref{}, based on figures documented in Schiek et al. [-@schiek2016demys].
...
The donor, USAID, originally awarded the LBr potato research contract to Cornell University in 2010 (?), with planned release in India. However, Cornell failed to complete stage 1 research successfully. In 2015, USAID transferred the contract to Michigan State University (MSU), which proposed a different stage 1 research strategy, with planned release in Bangladesh and Indonesia instead of India. India was dropped as a target country partly because the vigorous anti-transgenic lobby there made stage 4 success unlikely. MSU completed stage 1 successfully in 2019 (?), and USAID has recently awarded them a new contract for stage 2 research and development.
...
In real options language, USAID bought a 3-fold option on LBr potato when it awarded the original contract to Cornell; but this option expired out of the money at the end of stage 1. USAID then effectively bought a second $3$-fold option on LBr potato when it transferred the contract to MSU; and this option has now expired in the money.

In the present exercise, we calculate the 3-fold real option value of the USAID LBr potato contract awarded to MSU in 2015. This is the value of USAID's option, but not obligation, to continue on with funding of stage 2 of the 4 stage project. This is for purely pedagogical purposes hypothetical etc.<!-- using the algorithm provided at the end of the previous section.  -->

No published ex-ante impact assessments of LBr potato release in Bangladesh and Indonesia yet exist. For the purposes of this pedagogical exercise, say that MSU and/or USAID have internally conducted ex-ante impact assessments indicating a project NPV of $\$$`r x0`, but with a high degree of uncertainty ranging from $-$ `r 100 * yMin` $\%$ to $+$`r 100 * ymax`$\%$ of NPV. Based on these uncertainty bounds, the project NPV volatility parameter $s$ and growth rate $m$ are deduced following the method explained in section \ref{sec:volEst}.
...
#cv <- 7#6.5
#---------------------------------------------------------------------------
# Derive m, s, from min max pctg change elicited from experts and assumption 
# of lognormal log returns
...
Timpact <- 10
yrlyBen <- 1 * 10^6
X0 <- sum(yrlyBen / (1 + rYrly_discrete)^(tauN / 4 + Tadopt + 1:Timpact))
X0 <- 9 * 10^5 #9.8 * 10^5
#---------------------------------------------------------------------------
# Convert yearly discrete discount rate to quarterly continuous discount rate
rQtly_discrete <- (1 + rYrly_discrete)^(1 / 4) - 1
r <- log(1 + rQtly_discrete)
# rYrly <- log(1 + rYrly_discrete)
# r <- 0.018 #m
# m <- 0.02
# s <- m * cv
Bvec <- c(0, 0, 0)
