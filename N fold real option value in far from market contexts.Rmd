...
Then $E[x(T)]\bigr|_{t = 0} = e^{m T} x(0)$ (see Hull [-@hull9thEdition] for details); and inequality \ref{eq:npvIneq1} simplifies as follows.

\begin{equation}
e^{m - rT} x(0) - e^{-rT} K < S \:\: \rightarrow \:\: \text{reject project}
\label{eq:npvIneq2}
\end{equation}
...
Typically, $m$ is set equal to $r$ due to an artifact inherited from the financial context called "risk neutral valuation". Farther below, I demonstrate that risk neutral valuation has no place in far-from-market ROV contexts. There is thus no logical premise for setting $m = r$ here.
...
Which, by the formula for $\max(a, b)$, i.e.,
...
## The $n$-fold real option value of multi-stage AR4D projects
...
2) A basic replication or scaling up stage, whereby a technology proof-of-concept generated in the discovery stage---typically in the form of a genotype with a novel trait, plus a set of recommended farm management practices---is reproduced in greenhouse and/or confined field trials.

3) A multi-location, multi-season testing stage, whereby successful specimens generated in the preceding stage are taken for further scaling up and testing under distinct agronomic conditions over multiple cropping seasons. [mention marker assisted]

In the case of transgenic research, this is typically followed by:

4) A regulatory dossier stage, wherein detailed agronomic, environmental, and toxicological data, mostly generated during the preceding stages, is compiled into dossier(s) for submission to the National Competent Authorities (NCAs) in the target countries.

5) A deregulation stage, during which the regulatory dossier(s) generated during the previous stage is/are submitted to the NCA(s), which may request further clarification and testing of certain aspects of the proposed technology.

In today's increasingly holistic view of AR4D [@klerkx2010adaptive], it may also make sense to enumerate:

6) A release and uptake stage, during which the new technology is made available for distribution to target populations, typically some 2-3 decades after the start of stage 2. This stage depends critically on the quality of the socioeconomic enabling environment in the target countries, in conjunction with engagement from researchers to ensure correct management practices.

Donors typically define and fund stages 2-5 as a single project. They also engage with stages 1 and 6, but through separate arrangements, since investments at these stages generally apply across numerous prospective technologies. In some projects, it may make sense to divide stage 3 into two or more stages.
...
The real option value of multi-stage projects may be defined mathematically as follows. Let the real option value of a single stage project (equation \ref{eq:rovBraw}) evaluated at some time $\hat{t}$ henceforth be relabeled $f_1(\hat{t})$, with time horizon $T_1$, exercise cost $K_1$, and abandonment value $B_1$.

\begin{equation}
f_1(\hat{t}) = e^{-r \tau_1} E[\max(x(T_1) - K_1, B_1)] \bigr|_{t = \hat{t}}
\end{equation}

(Where $\tau_i = T_i - \hat{t}$.)

As explained above, this is the value of the option, but not the obligation, to disburse the funds $K_1$ required to implement the researched technology once the research project is finished. Now consider the real option value of a 2-stage project $f_2(\hat{t})$.

\begin{equation}
f_2(\hat{t}) = e^{- r \tau_2} E[\max(f_1(T_2) - K_2, B_2)] \bigr|_{t = \hat{t}}
\end{equation}

This is the value of the option, but not the obligation, to disburse the funds $K_2$ required to implement the second stage of the 2-stage project, once the first stage is finished. Note that $K_2$ corresponds to the sunk cost $S$ in equation \ref{eq:rovRaw}. The time horizon $T_2$ refers to the duration of stage 1, and $B_2$ is the abandonment value, if any, generated during stage 1.

Likewise, for a 3-stage project,

\begin{equation}
f_3(\hat{t}) = e^{- r \tau_3} E[\max(f_2(T_3) - K_3, B_3)] \bigr|_{t = \hat{t}}
\end{equation}
...
\begin{equation}
f_n(\hat{t}) = e^{- r \tau_n} E[\max(f_{n - 1}(T_n) - K_n, B_n)] \bigr|_{t = \hat{t}} \:\:;\:\:\: B_n \geq 0
\label{eq:rovNraw}
\end{equation}

In other words, $n$-fold option value is the present value of the option to continue with the subsequent stage of research, the value of which is itself the present value of the option to continue with the stage after that, and so forth, up until the project NPV $x(t)$.

Farther below, I demonstrate that, if $x(t)$ is lognormally distributed, then so is $f_n(t)$. Equation \ref{eq:part1} may therefore be invoked to establish a closed form expression for $f_n(\hat{t})$, if $x(t)$ is lognormally distributed. Before getting to this main result, however, it is first necessary to defend the assumption of lognormal $x(t)$, and to extract far-from-market real options from the thickets of "risk neutral valuation".

## [Low adoption of ROV due to complexity]

Despite a flood of academic interest in ROV following Myers' initial insight, adoption of the real options approach by real world decision makers remains relatively low [@horn2015use; @triantis2005realizing; @driouchi2012real]. ROV critics and proponents alike attribute this tepid reception to the formal complexity of evaluating and explaining ROV as compared to the default NPV approach [@triantis2005realizing].

Much of this complexity can be traced back to two sources. First of all, many authors reject the key assumption that project NPV evolution may be modeled as a gBm, preferring instead to evaluate equation \ref{eq:rovRaw} by numerical methods that are considerably less tractable, transparent, and instructive than equation \ref{eq:rov}. Secondly, there is confusion regarding the interpretation, in real options contexts, of the financial artifact known as "risk-neutral valuation". Before proceeding with the main work of this article, it is first necessary to redress these two sources of confusion in detail.
...
The time series on the left of Figure \ref{fig:NPVevol} is an example of geometric Brownian motion (gBm), while the time series on the right is a compound Poisson process (algorithm 6.2 in Cont and Tankov [-@]). The compound Poisson process (cPp) assumes that changes in value occur at random time intervals. This is fine in the financial context, but may be problematic in ROV contexts, where the information affecting project NPV is released through reports to management---events which are, almost by definition, non-random.

## In defense of the gBm model of project NPV

[Rejection of the gBm model in ROV contexts is partly inherited from the financial context, where gBm is generally viewed as a naive model of price movements. fat tails--debunked by Tankov, jumps--as discussed above, an artifact of the dam-break model, not a concern in ROV contexts, plus jumps can be closely approximated by gBm. en fin, gBm is much more versatile than it is generally given credit for.] Real options authors reject the gBm model of project NPV, preferring instead to select a model which they perceive to more accurately approximate the real evolution of project NPV. But this comes at a substantial loss of expedience, as the closed form expression in equation \ref{eq:rov} must be replaced by considerably less tractable expressions, up to and including numerical methods. Trigeorgis once characterized numerical methods as the unavoidable "bitter pill" that every serious ROV practitioner must come to grips with [-@trigeorgis1993real]. In hindsight, it is safe to say that energetic proponents of ROV like Trigeorgis may have overestimated the appetite for bitterness among non-academic audiences---enough so, at any rate, to warrant some revisiting of the matter:
...
In the financial context, the reality one aspires to model---i.e. some stochastic financial process, usually a price series---is well defined in the form of historical time series that can be easily downloaded, measured, analyzed, etc. In far-from-market R&D contexts, by contrast, analogous historical time series of project NPV generally do not exist. The reality one aspires to model must be perceived indirectly, based on assumptions and logic, as I have just done above. So, in far-from-market real options contexts, it is not even entirely clear what one gets in return for sacrificing expedience.

Then there is the question of the relevance of the increase in realism. In the financial context, the relevance of a realistic model of the time evolution of specific stochastic processes is clear. That is, it can make the difference between profit and loss. In far-from-market real options contexts, on the other hand, there is no real need for a good predictor of exactly when or in what order specific changes in project NPV occur. The relevant question is rather one of the size distribution of changes. In other words, it is not the time domain but the frequency domain that is of primary importance. The periodograms given in Figure \ref{fig:NPVevol2}, which correspond to the time series in the previous Figure, indicate that the size distribution of changes of the two time series is similar, despite their differences in the time domain.
...
This leads naturally to the final question of whether or not a comparable increase in realism, or the objective which the desired increase in realism is supposed to serve, at lower cost. ...the size distribution of changes generated by a gBm can approximate the size distribution of changes of a cPp arbitrarily closely by adjusting the parameters....and by judicious choice of time step.

[mention at top the cautionary tale against blackbox complexity of numerical methods -- the Majd Pyndyck paper error discovered by x some 40 yrs after the fact, with important repercussions for the results?]
...
The derivation of equation \ref{eq:rov} in the Appendix via the method of straightforward integration is atypical of the ROV literature. Most authors instead cite the Black-Scholes partial differentiation equation [-@black1973valuation] as the source of the ROV formula. The method of straightforward integration is considerably simpler than the Black-Scholes approach. In financial contexts, the Black-Scholes approach is advantageous because it generates a whole class of functional forms known as the "financial derivatives", of which the European call option formula (the financial analogue to equation \ref{eq:rov}) is just one.
...
Picking up again the thread in section ..., here I show that the $n$-fold real option value $f_n$ is lognormally distributed if project NPV $x(t)$ is lognormally distributed. A closed form expression for $f_n$ then follows from equation \ref{eq:rov}.

To begin, consider the far-from-market Black-Scholes PDE (equation \ref{eq:ffmBSpde}) for the 1-fold real option value function $f_1$, rearranged as an expression for $m f_1$.
...
\begin{equation}
s x \frac{\partial f_1}{\partial x} \epsilon \sqrt{\Delta t} = s f_1 \varepsilon_{1, 0} \epsilon \sqrt{\Delta t}
\end{equation}

Where $\varepsilon_{1, 0}$ is the elasticity of $f_1$ with respect to the project NPV $x$. That is, defining $f_0 = x$,

\begin{equation}
\begin{split}
\varepsilon_{n, 0} &= \frac{f_0}{f_n} \frac{\partial f_n}{\partial f_0} = \frac{\partial \ln(f_n)}{\partial \ln(f_0)} \\
...
\begin{equation}
\Delta f_1 = m f_1 \Delta t + s f_1 \varepsilon_{1, 0} \epsilon \sqrt{\Delta t}
\label{eq:gbmf1}
\end{equation}

By which it follows that $\Delta f_1 / f_1$ is normally distributed with mean $m \Delta t$ and variance $s^2 \varepsilon_{1, 0}^2 \Delta t$. From this, it follows that $f_1$ is a geometric Brownian movement, such that $\ln(f_1)$ is normally distributed with mean $\ln(f_1(\hat{t})) + \left(m - \frac{s^2 \varepsilon_{1, 0}^2}{2} \right) \tau_1$ and variance $s^2 \varepsilon_{1, 0}^2 \tau_1$.
...
\begin{equation}
\begin{split}
\Delta f_2 &= m f_2 \Delta t + s f_2 \varepsilon_{1, 0}  \frac{\partial f_2}{\partial f_1} \epsilon \sqrt{\Delta t} \\
&= m f_2 \Delta t + s f_2 \varepsilon_{2, 0} \epsilon \sqrt{\Delta t}
\end{split}
\label{eq:gbmf2}
\end{equation}

By which it follows that $\Delta f_2 / f_2$ is normally distributed with mean $m \Delta t$ and variance $s^2 \varepsilon_{2, 0}^2 \Delta t$; and that $\ln(f_2)$ is normally distributed with mean $\ln(f_2(\hat{t})) + \left(m - \frac{s^2 \varepsilon_{2, 0}^2}{2} \right) \tau_2$ and variance $s^2 \varepsilon_{2, 0}^2 \tau_2$. That is, $f_2$ is also a geometric Brownian movement.
...
\begin{equation}
\Delta f_n = m f_n \Delta t + s f_n \varepsilon_{n, 0} \epsilon \sqrt{\Delta t}
\label{eq:gbmfn}
\end{equation}

Such that $\Delta f_n / f_n$ is normally distributed with mean $m \Delta t$ and variance $s^2 \varepsilon_{n, 0}^2 \Delta t$, and $\ln(f_n)$ is normally distributed with mean $\ln(f_n(\hat{t})) + \left(m - \frac{s^2 \varepsilon_{n, 0}^2}{2} \right) \tau_n$ and variance $s^2 \varepsilon_{n, 0}^2 \tau_n$.

Remarkably, then, the mean percentage change in $n$-fold option value is equal to the mean percentage change in project value. The variance of percentage changes in $n$-fold option value similarly has the parameter $s$ in common with the variance of percentage changes in project NPV, but differs from the latter by a factor of $\varepsilon_{n,0}$, which changes in each time step.

Since $f_n$ is lognormally distributed, then, by equation \ref{eq:part1},

\begin{equation}
f_n = e^{(m - r) \tau_n} f_{n - 1}(\hat{t}) \Phi(\delta_n + s \varepsilon_{n - 1, 0} \sqrt{\tau_n}) - e^{-r \tau_n} K_n \Phi(\delta_n)
\label{eq:rovN}
\end{equation}
...
\begin{equation}
\delta_n = \frac{\ln \left(\frac{f_{n - 1}(\hat{t})}{K_n} \right) + \left(m - \frac{s^2 \varepsilon_{n - 1, 0}^2}{2} \right) \tau_n}{s \varepsilon_{n - 1, 0} \sqrt{\tau_n}}
\end{equation}

Which may be easily generalized to include abandonment value $B_n$ by following the steps in section ....
<!-- equation \ref{eq:rovB}, -->

\begin{equation}
f_n = e^{(m - r) \tau_n} f_{n - 1}(\hat{t}) \Phi(\delta_n + s \varepsilon_{n - 1, 0} \sqrt{\tau_n}) - e^{-r \tau_n} (K_n \Phi(\delta_n) - B_n ( 1 + \Phi(\delta_n)))
\label{eq:rovBn}
\end{equation}
...
\begin{equation}
\delta_n = \frac{\ln \left(\frac{f_{n - 1}(\hat{t})}{K_n - B_n} \right) + \left(m - \frac{s^2 \varepsilon_{n - 1, 0}^2}{2} \right) \tau_n}{s \varepsilon_{n - 1, 0} \sqrt{\tau_n}}
\end{equation}

In the case of $n = 1$, it is easy to see that this reduces to the 1-fold real option value formula in equation \ref{eq:rovB}.

For multi-stage projects ($n > 1$), equation \ref{eq:rovBn} must be evaluated recursively, starting with the $n^{th}$ stage. The steps of the algorithm are as follows:

1) Evaluate the real option value of the $n$th stage, $f_1$ and $\varepsilon_{1, 0}$.

2) Use $f_1$ and $\varepsilon_{1, 0}$ to evaluate $f_2$ and $\varepsilon_{2, 1}$.

3) Evaluate $\varepsilon_{2, 0} = \varepsilon_{2, 1} \varepsilon_{1, 0}$.

4) Repeat steps 2-3 up until $n$.

The elasticity $\varepsilon_{n, n - 1}$ works out to

\begin{equation}
\begin{split}
\varepsilon_{n, n - 1} &= \frac{f_{n - 1}}{f_n} e^{(m - r) \tau_n} (\Phi(\delta_n + s \sqrt{\tau_n}) + f_{n - 1} \frac{\partial \varepsilon_{n - 1, 0}}{\partial f_{n - 1}} \phi(\delta_n + s \sqrt{\tau_n})) \\
&= \frac{f_{n - 1}}{f_n} e^{(m - r) \tau_n} (\Phi(\delta_n + s \sqrt{\tau_n}) + (1 - \varepsilon_{n - 1, 0}) \phi(\delta_n + s \sqrt{\tau_n}))
\end{split}
\end{equation}
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \varepsilon_{n - 1, 0} &= \frac{\partial \ln(f_n)}{\partial \ln(f_{n - 1})} = \frac{f_{n - 1}}{f_n} \frac{\partial f_n}{\partial f_{n - 1}} \\ -->
<!-- &= e^{(m - r) T_n} \frac{f_{n - 1}}{f_n} \left( \Phi_{n, 1} + f_{n - 1} \phi_{n, 1} \frac{\partial \varepsilon_{n - 1, 0}}{\partial f_{n - 1}} s \sqrt{T_n} \right) \\ -->
<!-- &= \begin{cases} -->
<!-- 1, & n = 1 \\ -->
<!-- e^{(m - r) T_n} \frac{f_{n - 1}}{f_n} \left( \Phi_{n, 1} + f_{n - 1} \phi_{n, 1} \frac{\partial \varepsilon_{n - 1, 0}}{\partial f_{n - 1}} s \sqrt{T_n} \right), & n > 1 -->
<!-- \end{cases} -->
<!-- \end{split} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- mf_{n + 1} = \frac{\partial f_n}{\partial f_{n - 1}} f_{n - 1} m + \frac{\partial f_n}{\partial t} + \frac{s^2 f_{n - 1}^2 \varepsilon_{n - 1, 0}^2}{2} \frac{\partial^2 f_n}{\partial f_{n - 1}^2} -->
<!-- \label{eq:bsPDEn} -->
<!-- \end{equation} -->
<!-- note that by equation \ref{eq:bsPDEn},  -->

# An illustrative example of ROV valuation of an AR4D project

Below I apply the N-fold option technique derived above to valuate an AR4D project to develop a transgenic potato variety with resistance to Late Blight disease, for release as a public good in a single developing country. The project was implemented by Cornell University and Michigan State University under the USAID-ABSP II program, as documented in Schiek et al. [-@]. [Say something about the outcome of the project, how did it turn out?]
...
In stage 2, the transgenic events selected in stage 1 were cultivated in three distinct locations and three distinct seasons to assess environmental effects on the LBr trait, and vice versa. Any impacts of LBr on other key traits such as yield, maturation time, taste, and so forth, were also assessed.

In stage 3, the regulatory dossier was compiled for submission to the National Competent Authority in the country where the LBr potato was to be released. The dossier included compositional and safety assessments, as well as the environmental assessments of the previous stage.
...
The costs and time duration of each stage are summarized in Table 1. Costs are assessed as the sum of staff costs, direct operating costs (including lab bench costs), indirect operating costs (overhead), external contract costs, and stewardship costs.
...
Kvec <- c(181000, 312000, 336000, 530000)
stage1desc <- "Late Blight resistant event production and selection"
stage2desc <- "Wide area testing"
stage3desc <- "Compilation of the regulatory dossier"
stage4desc <- "Registration and regulatory affairs"

descVec <- c(stage1desc, stage2desc, stage3desc, stage4desc)

df_table <- data.frame(Stage = 1:4,
                       Kn = Kvec,
                       Duration = Tvec,
                       DurationCum = cumsum(Tvec),
                       Description = descVec)
...
table_title <- "Table 1: Project to develop a Late Blight resistant potato variety for release as a public good in 1 developing country."
#----------------------------------------------------------------------------
# Derive s
# lo <- 10^6
# hi <- 10^7
...
For the purposes of the present exercise, we transport ourselves back a couple years prior to the start of the LBr potato project, when the donor (USAID) is still deciding whether or not to fund the project. Alternatively, we could put ourselves in the shoes of a candidate (like Cornell University and/or Michigan State University) bidding on the project, and hence wishing to convince the donor of their ability to implement the LBr project such that project benefits (NPV) outweigh their costs.

In this hypothetical scenario, the foresight team advising senior level managers in charge of this decision assess the LBr project's NPV on the basis of improved food security associated with adoption of LBr potato in the target country. At the start of stage 1, this ($x(0)$) is estimated at `r x0` USD, with an expected quarterly growth rate ($m$) of `r m * 100` $\%$ (`r round(100 * ((1 + m)^4 - 1), 1)` $\%$ annually). Assuming that project NPV is a gBm, this means an expected NPV of `r exp(m * sum(Tvec)) * x0` by the end of the project (recall equation \ref...). However, there is considerable uncertainty regarding the enabling environment in the target country (i.e., the relevant policies, institutions, and markets), as well as research uncertainties, and so the foresight team sets the standard deviation ($s$) of project NPV growth at `r 100 * s` $\%$, or `r cv` times $m$.

[The discount rate is set to `r r` The discount rate is roughly the rate of capital growth in the target country. It stands to reason that the expected project NPV growth rate $m$ must be greater than $r$.]

The cost of implementing stage 1 is `r Kvec[N]` USD. In order to justify investment in the project, it must be shown that the ROV of stage 1 is greater than this cost. [Since the investor is a not-for-profit entity and expects to subsidize... some level of loss may be acceptable, let's say 10% below the implementation cost is ok.]


...the aim of this exercise is to place a value on the option to either continue on to stage 2 or to abandon the project. The option values of the other stages are calculated in the process, and so are also reported.
...
# Calculate ROV
#N <- 4
X0 <- 10.4 * 10^5 #9.8 * 10^5
#Kvec <- 1.5 * runif(N) * X0
#Kvec <- c(180541, 311974, 335530, 530250)
Kvec <- c(181000, 312000, 336000, 530000)
#Kvec <- c(52000, 213000, 396000, 929000)
N <- length(Kvec)
#Tvec <- sample(1:4, N, replace = T) * 4
#Tvec <- c(3, 0.83, 1.67, 3.42) * 4
Tvec <- c(3, 0.85, 1.7, 3.4) * 4
...
Bvec <- c(0, 0, 50000)
df_wB <- rovN(X0, Kvec, Bvec, Tvec, s, m, r)
ROV_wB <- df$OVn[nrow(df)]
dif_wB <- ROV_wB - KN
#--------------------------------------------------------------------------
# Compare to conventional NPV
#NPV <- exp(-r * sum(Tvec)) * (X0 - sum(Kvec))
NPV <- exp((m - r) * sum(Tvec)) * X0 - sum(exp(-r * rev(cumsum(rev(Tvec)))) * (Kvec - Bvec))
dif_npv <- NPV - KN
#--------------------------------------------------------------------------
...
The donor is a not-for-profit entity, and expects to subsidize or "insure" the project to some extent (within reason), and so even a negative result would be acceptable, so long as it is not too negative. Say the rule should be that it is above a -10% ROI.

Say that part of the stage 1 investment goes toward a permanent upgrade of personnel and lab/greenhouse equipment in the competent national agricultural research institution of the target country, valued at `r Bvec[N]` USD. This value will be realized whether or not stage 1 is successful. Then, using equation \ref..., the ROV margin becomes `r round(dif_wB)`, and the probability of stage 1 success increases to `r 100 * round(df_wB$Phi2, 2)` $\%$.

`r round(dif_npv)`

Given the uncertainty surrounding not just project NPV, but also the parameters, point estimates such as this are of limited use to decision makers. [We do not actually know what the ]    Better a map...instant sensitivity analysis. 

```{r, echo=FALSE}

# ROV Map
n <- 20
X0vec <- seq(0.75, 1.2, length.out = n)
cvVec <- seq(2, 15, length.out = n)
#----
Kvec <- c(181000, 312000, 336000, 530000)
N <- length(Kvec)
Tvec <- c(3, 0.85, 1.7, 3.4) * 4
...
difmat <- matrix(NA, n, n)
difNPVmat <- matrix(NA, n, n)
phi2mat <- matrix(NA, n, n)
for(i in 1:n){
    X0 <- X0vec[i] * 10^6
  for(j in 1:n){
    cv <- cvVec[j]
    s <- m * cv
      df <- rovN(X0, Kvec, Bvec, Tvec, s, m, r)
  ROV <- df$OVn[nrow(df)] * 10^-5
  NPV <- exp((m - r) * sum(Tvec)) * X0 - sum(exp(-r * rev(cumsum(rev(Tvec)))) * (Kvec - Bvec))
    difmat[i, j] <- ROV - KN
  phi2mat[i, j] <- df$Phi2[nrow(df)]
  difNPVmat[i, j] <- NPV - KN
...
colnames(difmat) <- as.character(cvVec)
row.names(difmat) <- as.character(X0vec)
df_plot <- reshape2::melt(difmat)
colnames(df_plot) <- c("Project NPV at t = 0 (million USD)", "Uncertainty (coeff. of variation)", "ROV net benefit\n(100 thousand USD)")
colnames(phi2mat) <- as.character(cvVec)
row.names(phi2mat) <- as.character(X0vec)
df_plot <- reshape2::melt(phi2mat)
colnames(df_plot) <- c("Project NPV at t = 0 (million USD)", "Uncertainty (coeff. of variation)", "Probability of success")
#, "NPV net benefit", )

#---
gg <- ggplot(df_plot, aes(x = `Uncertainty (coeff. of variation)`,
...
<!-- \begin{equation} -->
<!-- \ln \left(\frac{f}{f\bigr|_{t = 0}}  \right) ~ \phi \left(\left(m - \frac{s^2 \varepsilon_{f, x}^2}{2} \right) T, s^2 \varepsilon_{f, x}^2 T \right) -->
<!-- \end{equation} -->

<!-- The function $f$ of the geometric Brownian motion $x$ is thus itself a geometric Brownian motion. That is to say, the log returns of $f$ are normally distributed with mean $\left(m - \frac{s^2 \varepsilon_{f, x}^2}{2} \right) T$ and variance $s^2 \varepsilon_{f, x}^2 T$. -->
...
<!-- \begin{equation} -->
<!-- \Delta g = \left( \frac{\partial g}{\partial f} f m + \frac{\partial g}{\partial t} + \frac{s^2 f^2 \varepsilon_{f, x}^2}{2} \frac{\partial^2 g}{\partial f^2} \right) \Delta t + s f \varespsilon_{f, x} \frac{\partial g}{\partial f} \epsilon \sqrt{\Delta t} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- mg = \frac{\partial g}{\partial f} f m + \frac{\partial g}{\partial t} + \frac{s^2 f^2 \varepsilon_{f, x}^2}{2} \frac{\partial^2 g}{\partial f^2} -->
<!-- \label{eq:bsPDEg} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \frac{\Delta g}{g} ~ \phi(m \Delta t, s^2 \varepsilon_{g, x}^2 \Delta t) -->
<!-- \end{equation} -->

<!-- And -->

<!-- \begin{equation} -->
<!-- \ln \left(\frac{g}{g \bigr|_{t = 0}}  \right) ~ \phi \left(\left(m - \frac{s^2 \varepsilon_{g, x}^2}{2} \right) T, s^2 \varepsilon_{g, x}^2 T \right) -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \Delta f_n = m f_n \Delta t + s \varepsilon_{n, 0} \epsilon \sqrt{\Delta t} -->
<!-- \label{eq:gBmN} -->
<!-- \end{equation} -->

<!-- For $n = 0, 1, 2$. (Where, to be clear, $\varepsilon_{n, 0} = \frac{\partial \ln(f_n)}{\partial \ln(f_0)}$.) -->
...
<!-- \begin{equation} -->
<!-- mf_n = \frac{\partial f_n}{\partial f_{n - 1}} f_{n - 1} m + \frac{\partial f_n}{\partial t} + \frac{s^2 f_{n - 1}^2 \varepsilon_{n - 1, 0}^2}{2} \frac{\partial^2 f_n}{\partial f_{n - 1}^2} -->
<!-- \label{eq:bsPDEn} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \Delta f_{n + 1} = \left( \frac{\partial f_{n + 1}}{\partial f_n} f_n m + \frac{\partial f_{n + 1}}{\partial t} + \frac{s^2 f_n^2 \varepsilon_{n, 0}^2}{2} \frac{\partial^2 f_{n + 1}}{\partial f_n^2} \right) \Delta t + s f_n \varespsilon_{n, 0} \frac{\partial f_{n + 1}}{\partial f_n} \epsilon \sqrt{\Delta t} -->
<!-- \label{eq:gbmNproof1} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- mf_{n + 1} = \frac{\partial f_{n + 1}}{\partial f_n} f_n m + \frac{\partial f_{n + 1}}{\partial t} + \frac{s^2 f_n^2 \varepsilon_{n, 0}^2}{2} \frac{\partial^2 f_{n + 1}}{\partial f_n^2} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \frac{\Delta f_{n + 1}}{f_n} ~ \phi(m \Delta t, s^2 \varepsilon_{f_{n + 1}, f_n}^2 \Delta t) -->
<!-- \label{eq:gBmNp1} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \ln \left(\frac{f_{n + 1}}{f_{n + 1} \bigr|_{t = 0}}  \right) ~ \phi \left(\left(m - \frac{s^2 \varepsilon_{n + 1, 0}^2}{2} \right) T_{n + 1}, s^2 \varepsilon_{n + 1, 0}^2 T_{n + 1} \right) -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- f_n = e^{(m - r) T_n} (f_{n - 1} \Phi(\delta_n + \varepsilon_{n - 1, 0} s \sqrt{T_n}) - e^{-r T_n} K_n) -->
<!-- \end{equation} -->

<!-- Where -->

<!-- \begin{equation} -->
<!-- \delta_n = \frac{\ln \left( \frac{f_{n - 1}}{K_n} \right) + \left( m - \frac{s^2 \varepsilon_{n - 1, 0}^2}{2} \right) T_n}{s \varepsilon_{n - 1, 0} \sqrt{T_n}} -->
<!-- \end{equation} -->
...
<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \varepsilon_{n - 1, 0} &= \frac{\partial \ln(f_{n - 1})}{\partial \ln(f_0)} = \frac{f_0}{f_{n - 1}} \frac{\partial f_{n - 1}}{\partial f_0} \\ -->
...
<!-- \begin{equation} -->
<!-- s x \frac{\partial f_n}{\partial x} \epsilon \sqrt{\Delta t} = s f_n \varepsilon_{f, x} \epsilon \sqrt{\Delta t} -->
<!-- \end{equation} -->

<!-- Where $\varepsilon_{n, 0}$ is the elasticity of $f_n$ with respect to the project NPV $x$. That is, -->

<!-- \begin{equation} -->
<!-- \begin{split} -->
<!-- \varepsilon_{n, 0} &= \frac{x}{f_n} \frac{\partial f_n}{\partial x} = \frac{\partial \ln(f_n)}{\partial \ln(x)} \\ -->
...
<!-- By which it follows that $\Delta f_n / f_n$ is normally distributed with mean $m \Delta t$ and variance $s^2 \varepsilon_{n, 0}^2 \Delta t$. From this, it follows that $f_n$ is a geometric Brownian movement, such that $\ln(f_n)$ is normally distributed with mean $\ln(f_n(\hat{t})) + \left(m - \frac{s^2 \varepsilon_{n, 0}^2}{2} \right) \tau_n$ and variance $s^2 \varepsilon_{n, 0}^2 \tau_n$. -->
