...
abstract: |
  I adapt real options valuation to multi-stage, far-from-market, high risk, high reward projects (such as agricultural research for development projects). After a brief introduction to the basics of real options valuation, including an extension to projects with abandonment value, I proceed to derive a novel real options valuation model for far-from-market, multi-stage projects. To prepare the ground for this derivation, I first carefully adapt the key assumption of lognormally distributed returns to project net present value to the agricultural research for development context. And I argue that the entrenched assumption of risk-neutral valuation is inappropriate in far-from-market contexts, and demonstrate the mathematical implications of non-risk-neutrality for real options valuation. Based on this groundwork, I then derive the multi-stage or "$n$-fold", far-from-market real option value model. Finally, I walk through an illustrative example of how the model might be used to valuate a real, 4-stage transgenic potato project.
...
Since much of the target audience is probably unfamiliar with the real options literature, I build up to these results incrementally, starting with an introduction to the basic concept. While AR4D is the motivating context for the present work, the $n$-fold ROV model derived here is broadly applicable to any high risk, high reward, multi-stage, far-from-market, real option context.
<!-- * R script included -->
...
```{r Fig1, fig.show = "hold", fig.width = 3, fig.height=4, fig.align="center", fig.cap="\\label{fig:rovIllust}A hypothetical project's real option value and net present value plotted together over a range of moneyness values. The black line marks 0, while the dotted red line marks the sunk cost $S$ beneath which the project is rejected. The multiple ROV plots correspond to different levels of uncertainty (s). The higher the uncertainty, the greater the difference between ROV and NPV.", echo = FALSE}
...
The coefficient of variation $s \over m \sqrt{\tau}$ is a handy, normalized measure of project NPV volatility over time. One should not be surprised to see high coefficients of variation in the AR4D context. Because AR4D projects are usually deep in the money, a low coefficient of variation may indicate that ROV will not differ significantly from NPV, and that there is thus no point in calculating ROV.
...
In the present exercise, I use the $n$-fold ROV model derived in the previous section to calculate the 4-fold real option value of the USAID LBr potato contract awarded to MSU in 2015. This is the value of USAID's option, but not obligation, to fund stage 2 (multi-location, multi-season confined field trials) once stage 1 is complete. In order to make the calculation, the model requires the research stage costs $K_1, K_2, \dots, K_4$ and time horizons $T_1, T_2, \dots, T_4$ listed in Table 1, as well as:

* The post-project launch cost $K_0$

* The project NPV $x(0)$

* The project NPV growth rate $m$ and volatility $s$, or the upper and lower bounds on project NPV, whereby $m$ and $s$ can be deduced

* Any abandonment values associated with each stage $B_1, B_2,\dots, B_4$
...
yMax <- log(1 + 2.55) #2.55 #1.31
yMin <- log(1 - 0.85) #-0.85 #-0.88
...
The research stage costs and time horizons used in the calculation are based on Cornell's and MSU's real experiences with LBr potato research [@schiek2016demys]. The other parameters must be filled in with hypothetical values.
...
No ex-ante impact assessments of LBr potato in Bangladesh and Indonesia have yet been published. For the pedagogical purposes of this exercise, say that MSU and/or USAID internally conducted an ex-ante impact assessment indicating an LBr project NPV of $x(0)=\$$`r x0`, with a high degree of uncertainty ranging from $-$ `r 100 * yMin` $\%$ to $+$`r 100 * yMax`$\%$ of NPV. Based on these uncertainty bounds (and assuming project NPV follows a gBm), the project NPV volatility parameter $s$ and quarterly growth rate $m$ are deduced following the method explained in section \ref{sec:volEst}. These work out to $m = $ `r m` (`r round(((1 + m)^4 - 1), 1)` annually) and $s = $ `r s`.
...
This is a purely pedagogical exercise to illustrate the use of the $n$-fold ROV model derived in this article. While the exercise is based on real project costs and time horizons, the other parameter values are hypothetical. The resulting calculations should therefore not be interpreted as an authoritative valuation of LBr potato research.
...
In this article, I have proposed a real options valuation workflow for multi-stage AR4D projects, built around a novel $n$-fold ROV model. The $n$-fold model...

In the example provided above, the project is a transgenic project following a 4-stage process outlined in section \ref.... The precise breakdown of stages may vary in other projects. In some cases, for example, it may make sense to divide stage 2 into two or more stages. The method can be applied just as well to conventional breeding projects, although the particular stages have to be determined case by case. livestock too.

* The proposed workflow is beneficial for researchers and donors alike. From the researchers' perspective, the proposed approach is appealing for two reasons: 1) The ROV approach can account for the wide uncertainty inherent in far-from-market research. Researchers, with good reason, generally resent being forced to place a single value on impacts that may be 15-30+ years off in the future [@Petsko2011; @Moriarty2008; @leeuwis2018reforming]. The proposed workflow allows researchers to accompany such point estimates with a wide error range. 2) The ROV approach prices in the donor's prerogative to discontinue funding of under-performing projects. Donors have long had this prerogative, de facto, but conventional methods of project appraisal omit the value it adds. 3) An accounting mechanism that generally facilitates the long term financial commitments required to fund disruptive research.

* From the donors' perspective, the proposed approach is appealing because it provides a way to rigorously justify commitments to long time horizon projects while at the same time preserving the flexibility necessary... in increasingly austere fiscal constraints.

* The exposition here focuses on transgenic research because the staging of such projects tends to follow the generic structure described in section \ref{sec:}, regardless of the crop or technology being researched. The proposed workflow applies equally well to conventional breeding projects, which are also multi-staged, but the staging may vary considerably from one crop and technology to another. Details of the staging and cost assessment may also vary depending upon whether the technology is to be released as a public good, as in the case of LBr potato, or under private contract. The method applies to livestock projects as well.    ... Not limited to AR4D, but applies to any high risk, high reward, far-from-market, multi-stage real option context. The model may also be applied in near-market settings, but in that case the expected instantaneous project return $m$ should be set equal to $r$, to reflect risk-neutral valuation. For projects with a high expected reward, and little risk, there is probably no point in using the ROV approach, as the output will not differ significantly from the output of a conventional CBA approach. [The higher the ratio of benefits to costs, the higher must be the uncertainty for there to be a meaningful difference between ROV and NPV.]
...
* For probabilities close to 1, the ROV will differ little from the default NPV approach.  If high reward projects are not also high risk, there is no point in assessing ROV, as the result will not differ significantly from NPV.


Defense of assumption that project NPV follows a gBm

I argued that the assumption of risk-neutral valuation is inappropriate in far-from-market contexts like AR4D, and shown that this results   which I derived from   a based on the assumption that project NPV follows a gBm. I have argued that it is inappropriate to assume risk neutral valuation in the AR4D context, and that 
...
For projects with high expected net payoff, the probability of project success $\Phi(\delta_N)$ will effectively be $1$---and hence ROV will not differ meaningfully from NPV---unless there is also high uncertainty surrounding the expected payoff.    In the example above [refer to decision map figure], the ratio is so high that project ROV is virtually the same as NPV for a coefficient of variation less than $5$. 
