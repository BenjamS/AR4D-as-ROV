---
title: Real option value in extreme risk, multi-staged, not for profit, far from market contexts
author:
...
Here I... projects are characterized by: long, multi-staged time horizons, extreme payoff, extreme uncertainty (because of long time horizons), far from market

* Extremely high reward, but also extremely high risk---risk from the research side, as well as the risk that inevitably comes with such long time horizons.  as well as risk arising from unresolved questions about the scope of the proposed tech, and/or range of applications, which cannot be resolved until the project is well underway. Talking about "game changing" technology, analogous to the laser.

* far from market (risk neutral valuation does not apply)

* Confusion regarding risk neutral valuation in real options contexts

* Modeling the evolution of project NPV as gBm

* Easy extension to abandonment value

* N-fold options (more transparent, intuitive alternative to the model presented by Cassimons et al., plus easily extensible to include abandonment value.)

* Important motivating factor: In today's world, increasingly fraught with multi-dimensional uncertainty and peril, donors cannot afford politically or financially to commit to long time horizons. Flexibility to pull out attractive to them... ROV, and especially N-fold ROV, presents a pathway by which donors can effectively fund long term projects without the long term commitment.

* R script included

* I use the derived formula to valuate a transgenic potato AR4D project.
...
The term $\Phi(d)$ is very instructive. It is the probability that $x(T) > K$---in other words, the probability of project success. To see this, note that...

The term $\Phi(d + s \sqrt{\tau})$ is also instructive. It is equal to the partial derivative $\partial ROV / \partial x$, and thus provides insight into the sensitivity of real option value with respect to movements in the underlying project NPV.
...
In the financial context, the reality one aspires to model---i.e. some stochastic financial process, usually a price series---is well defined in the form of historical time series that can be easily downloaded, measured, analyzed, etc. In far-from-market R&D contexts, by contrast, analogous historical time series of project NPV generally do not exist. The reality one aspires to model must be perceived indirectly, based on assumptions and logic, as I have just done above. So, it is not even entirely clear what one gets in return for sacrificing expedience.

Then there is the question of the relevance of the increase in realism. In the financial context, the relevance of a realistic model of the time evolution of specific stochastic processes is clear: it can make the difference between profit and loss. In far-from-market real options contexts, on the other hand, there is no real need for a good predictor of exactly when or in what order specific changes in project NPV occur. The relevant question is rather one of the size distribution of changes. In other words, it is not the time domain but the frequency domain that is of primary importance. The periodograms given in Figure \ref{fig:NPVevol2}, which correspond to the time series in the previous Figure, indicate that the size distribution of changes of the two time series is similar, despite their differences in the time domain.
...
Black and Scholes solved this PDE under the boundary condition $f(T) = \max(x(T) - K, 0)$, resulting in an expression for $f$ identical to the one derived above in equation \ref{eq:EmaxTK}, _with the exception_ that $m$ is replaced with $r$.

But Black and Scholes' insight can be decomposed into two consecutive insights. The first insight is that, having eliminated the random terms in equation \ref{eq:bsInsight1}, the now deterministic equation implies the evolution of the lefthand side must be constant over the interval $\Delta t$. This means the evolution in terms of $\Delta f$ and $\Delta x$ on the lefthand side of equation \ref{eq:bsInsight1} may be rewritten in terms of a time evolution, as follows.
...
# N-fold real option value

When ROV-valuating far-from-market R&D project, valuating the first stage of the project as the option to continue with the second stage. If at the end of stage 1 donors/managers decide to exercise the option to continue with stage 2, then the ROV of stage 2 is the value of the option, but not the obligation, to continue on to stage 3.

Consider now an option on an option, i.e. an option whose underlying variable is the option value $ROV$ in equation \ref{eq:rov}. Let the real option value of $f_1$ be called $f_2$, and relabeling $f_1 = ROV$ to reduce clutter,

\begin{equation}
ROV_2 = e^{-(m - r) T_2} E[\max(ROV_1(T_2) - S, B_2)]
\end{equation}



The sunk cost $S$ becomes the exercise price $K_1$...
...
\begin{equation}
mf = \frac{\partial f}{\partial x} x m + \frac{\partial f}{\partial t} + \frac{s^2 x^2}{2} \frac{\partial^2 f}{\partial x^2}
\label{eq:bsPDEf}
\end{equation}

Note that the righthand side of this expression is equal to the first term in parenthesis on the righthand side of Ito's Lemma in equation \ref.... Therefore, Ito's Lemma, in this case, reduces as follows.

\begin{equation}
\Delta f = m f \Delta t + s x \frac{\partial f}{\partial x} \epsilon \sqrt{\Delta t}
\end{equation}

The second term on the righthand side can also be reduced as follows.

\begin{equation}
s x \frac{\partial f}{\partial x} \epsilon \sqrt{\Delta t} = s f \varepsilon_{f, x} \epsilon \sqrt{\Delta t}
\end{equation}

Where $\varepsilon_{f, x}$ is the elasticity of real option value $f$ with respect to the underlying project NPV $x$. That is,

\begin{equation}
\begin{split}
\varepsilon_{f,x} &= \frac{x}{f} \frac{\partial f}{\partial x} = \frac{\partial \ln(f)}{\partial \ln(x)} \\
&= \frac{1}{100} \frac{%\Delta f}{%\Delta x}
\end{split}
\end{equation}

Equation \ref... (Ito's Lemma) can thus be rewritten

\begin{equation}
\Delta f = m f \Delta t + s f \varepsilon \epsilon \sqrt{\Delta t}
\label{eq:gbmf}
\end{equation}
...
\begin{equation}
\frac{\Delta f}{f} ~ \phi(m \Delta t, s^2 \varepsilon_{f, x}^2 \Delta t)
\end{equation}
...
In the case of $n = 1$, it is easy to see that the above formula reduces to the (1-fold) real option value formula in equation \ref{eq:rovRaw}.

Remarkably, equations \ref{eq:gBmX} and \ref{eq:gBmN} imply that the mean percentage change in N-fold option value is equal to the mean percentage change in project value. N-fold option value variance likewise has the parameter $s$ in common with project NPV variance, but differs from the latter by a factor of $\varepsilon_{n,0}$, which changes in each time step.

The elasticity $\varepsilon_{n,0}$ can be evaluated recursively as follows.

1) Evaluate the 1-fold real option value $f_1$ and $\varepsilon_{1, 0}$.

2) Use $f_1$ and $\varepsilon_{1, 0}$ to evaluate $f_2$
...
Below I apply the N-fold option technique derived above to valuate an AR4D project designed to develop a transgenic potato variety with resistance to Late Blight (LB) disease, for release as a public good in a single developing country. The project was implemented by Cornell University and Michigan State University under the USAID-ABSP II program, as documented in Schiek et al. [-@]. [Say something about the outcome of the project, how did it turn out?]

The project occurred in 4 distinct stages: 1) Late Blight resistant event production and selection, 2) Wide area testing, 3) Compilation of the regulatory dossier, and 4) Registration and regulatory affairs.

In stage 1, the lead gene construct is developed in a target potato variety, and a small number of transgenic events exhibiting high resistance to Late Blight, as well as a number of other qualities (absence of backbone vector sequences, minimum number copy number of R genes, etc.) are screened from a large number of explants.

In stage 2, the transgenic events selected in stage 1 are cultivated in three distinct locations and three distinct seasons to assess environmental effects on the LBr trait, and vice versa. Any impacts of LBr on other key traits such as yield, maturation time, taste, and so forth, are also assessed.

In stage 3, the regulatory dossier is compiled for submission to the National Competent Authority in the country where the LBr potato is to be released. The dossier includes compositional and safety assessments, as well as the environmental assessments of the previous stage.

In stage 4, the regulatory dossier is defended and amended before the National Competent Authorty in the target country. This may involve a variety of activities, including public advocacy, lobbying, and submission of additional information.

The costs and time duration of each stage are summarized in Table 1. Costs are assessed as the sum of staff costs, direct operating costs (including lab bench costs), indirect operating costs (overhead), external contract costs, and stewardship costs. For details see Schiek et al. [-@].
...
In this hypothetical scenario, the foresight team advising senior level managers in charge of this decision assess the LBr project's NPV at the start of stage 1 ($x(0)$) at `r x0` USD, with an expected quarterly growth rate ($m$) of `r m * 100` $\%$ (`r round(100 * ((1 + 0.006)^4 - 1), 1)` $\%$ annually). Assuming that project NPV is a gBm, this means an expected NPV of `r exp(m * sum(Tvec)) * x0` by the end of the project (recall equation \ref...). However, there is considerable uncertainty regarding the enabling environment in the target country (i.e., the relevant policies, institutions, and markets), as well as research uncertainties, and so the foresight team sets the standard deviation ($s$) of project NPV growth at `r 100 * s` $\%$, or `r cv` times $m$.

[The discount rate is set to `r r`]
...
Given the uncertainty surrounding not just project NPV, but also the parameters, point estimates such as this are of limited use to decision makers. Better a map...instant sensitivity analysis. 
