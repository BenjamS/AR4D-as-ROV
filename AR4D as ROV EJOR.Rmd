...
Project NPV $x(t)$ is defined as post-R&D project net benefits discounted back to the completion and release of the research product at time $T_0$, typically determined by ex-ante impact assessment. Time $t < T$ is the time of the assessment. Assuming $x(t)$ follows a gBm, then its evolution $\Delta x$ over a small time increment $\Delta t$ is described by
...
In the case where $q = x(T_0)$, $C = K_0$, and multiplying through by the discount factor, this becomes
...
\begin{equation}
E\left[\frac{\Delta f(\tilde{t})}{f(t)} \right]\bigr|_{t < \tilde{t}} = \left(r - \eta_{1,0}^2 \frac{s^2}{2} \right) \tau
\end{equation}

\begin{equation}
Var\left[\frac{\Delta f(\tilde{t})}{f(t)} \right]\bigr|_{t < \tilde{t}} = \eta_{1,0}^2 s^2 \tau
\end{equation}
...
\begin{equation}
\delta_1 = \frac{ \ln \left(\frac{f(t)}{K_1} \right) - \left(r - \eta_{10}^2 \frac{s^2}{2} \right) \tau_g} {\eta_{1,0} s \sqrt{\tau_g}}
\end{equation}
...
\begin{equation}
E\left[\frac{\Delta g(\tilde{t})}{f(t)} \right]\bigr|_{t < \tilde{t} \leq T_1} = \left(r - \eta_{2,0}^2 \frac{s^2}{2} \right) \tau_2
\end{equation}

\begin{equation}
Var\left[\frac{\Delta f(\tilde{t})}{f(t)} \right]\bigr|_{t < \tilde{t} \leq T_1} = \eta_{2,0}^2 s^2 \tau_2
\end{equation}
