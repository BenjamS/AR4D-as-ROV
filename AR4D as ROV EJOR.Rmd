...
In this article, we develop an ROV method for the AR4D context. Up until now, ROV methods have developed in the context of corporate projects. A simple extension of this work to the AR4D context is not possible due to the many key differences between the two contexts. In particular, corporate projects unfold in market or near market settings, where much of the original option value theory developed in finance may be invoked. AR4D projects, on the other hand, unfold in the midst of market failures, where no such theory may be invoked. The first step in the present work is thus a studious avoidance of the theoretical underpinnings of existing ROV methods, and the development of suitable foundations in their place.

The second step is to develop a multi-stage ROV method corresponding to the multi-stage structure of most AR4D projects. For example, many AR4D plant breeding projects involve a preliminary greenhouse and/or field trial(s) stage, followed by more advanced, multi-location field trials [@covarrubias2022breeding]. In transgenic projects, additional toxicological testing is required, followed by the compilation of a regulatory dossier and application for deregulation in the target countries where the new technology is to be released. Finally, there is the release, distribution, and outscaling stage, which involves careful orchestration of complex networks of stakeholders and public-private partnerships. Meanwhile, existing ROV methods focus almost exclusively on single stage projects. The one exception is a multi-stage model proposed by Cassimon et al. [-@cassimon2004valuation] for the evaluation of pharmaceutical trials, building upon the "compound option" model originally developed by Geske in the financial context [-@geske1979valuation]. Again, this work is not readily extensible to AR4D due to its corporate and financial theoretical underpinnings. Here we develop a multi-stage ROV model upon the suitable foundations laid in the first step.
...
Steps one and two constitute the main work of this article. Additionally, two elementary, but no less novel and important, adjustments are introduced in order to accommodate two more key features of the AR4D context not captured by existing ROV methods: abandonment value and the marginally diminishing relationship between project impact and the outscaling investment.

By "abandonment value", we mean the value often generated by AR4D projects regardless of the success or failure of the main research objectives. A project might generate abandonment value in the form of upgraded research facilities or improved human capital through training, for example. As for the "scaling of impacts with investment", we refer to the simple scaling relationship that typically exists between AR4D project NPV and the level of investment in the distribution and outscaling of the finished technology. As the level is greater, more potential beneficiaries are reached, and hence the expected impact and corresponding NPV are greater. Existing methods assume independence between project NPV and investment. Here we model project NPV as a marginally diminishing function of the outscaling investment [that occurrs in the last stage]. [...introduces the possibility of optimizing the outscaling investment]

Since much of the intended audience is probably unfamiliar with ROV, we build up to these results incrementally, starting with a formal introduction to the basic concept, intuition, and a straightforward derivation of the ROV equation for 1-stage projects, stripped of all financial trappings. From here move to the multi-stage ...Finally, we apply the derived model to appraise a real 4-stage AR4D potato project.
...
The left hand side of this inequality is the project ROV. Note that ROV is partly a function of NPV. In the ROV literature, ROV is sometimes characterized as an alternative to the "NPV approach". This is misleading insofar as it suggests that ROV is an alternative to ex-ante impact assessment. On the contrary, ROV requires ex-ante impact assessment.... the formal exposition so far demonstrates that ROV is a function of NPV, and thus involves its estimation (and this becomes clearer as the formal exposition continues, below). For this reason, the conventional or default approach to project funding decisions characterized in Inequality \ref{eq:convCBA}, against which the ROV approach is distinguished, is henceforth referred to as the "conventional CBA criterion"---and not the "NPV approach".
