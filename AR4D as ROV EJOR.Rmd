...
In the present setting, project NPV $x(t)$ is defined as post-R&D project net benefits discounted back to the date of release ($T$) of the research product. The time $t < T$ is the time at which project NPV is evaluated. By definition, then, $E[x(T)]\bigr|_{t} = x(t)$. Or, for any intermediate time $t < \tilde{t} < T$, $E[x(\tilde{t})]\bigr|_{t} = x(t)$.

The present value of project NPV at time $t$ is just $x(t)$ discounted back to $t$. This may be expressed $PV(x)\bigr|_{t} = e^{r \tau} x(t)$, where $r$ is the discount rate and $\tau = T - t$.
...
Where $\epsilon$ is a normally distributed random variable with mean $0$ and variance $1$, such that constants $m$ and $s$ quantify the trend and volatility of the time evolution, respectively.
...
The mean and variance of the log change over any finite interval $\tau = \tilde{t} - t$ is given by

\begin{equation}
\begin{split}
E \left[ \frac{x(\tilde{t})}{x(t)} \right]  &= \left(m - \frac{s^2}{2} \right) \tau  \\
Var \left[ \frac{x(\tilde{t})}{x(t)} \right] &= s^2 \tau \right]
\end{split}
\end{equation}
