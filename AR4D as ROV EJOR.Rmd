...
It is worth noting, moreover, that many donors already acquire an unacknowledged option value in their conventional research funding arrangements. That is to say, nowadays, most AR4D contracts are signed for no more than 1-5 years, at the end of which the contract comes up for renewal or cancellation, pending progress up to that point. By reserving the right to discontinue funding at specific time intervals, the donor effectively acquires option value. The introduction of ROV would thus merely formalize an already existing practice, and more accurately quantify the real value acquired by the donor in exchange for their investment. By the same token, default methods of cost benefit analysis neglect to price in the donor's option value, and thus understate the real value acquired by the donor.

Up until now, ROV methods have developed in the context of corporate projects. Extension of these methods to the AR4D context is complicated by a number of methodological challenges stemming from the key difference between the two contexts: corporate projects occur in market or near market settings, whereas AR4D projects are born of market failures. Methodologically, this means that corporate project NPV can be estimated as a linear combination of relevant prices---by invoking a financial argument called "replicating portfolios" [@brennan1985evaluating; @koller2010valuation; @amram1998real].

Insofar as the replicating portfolios argument is valid, it greatly facilitates the evaluation of project NPV and risk---both of which are required to evaluate project ROV---as NPV may then be easily calculated at any time from publicly available price information; and project risk may be assessed by inspecting the price time series. Some might argue that the recent emergence of carbon and green bond markets [@tolliver2019green; @bhutta2022green] paves the way for a replicating portfolios analogue in the AR4D context. However, in addition to green outcomes, AR4D projects typically focus on health, nutrition, and socioeconomic outcomes for marginalized populations, the respective values of which, almost by definition, are not reflected in any market. And, even if they were---even if there were an "SDG bond" mapped to every AR4D impact area---this might be useful in the evaluation of project NPV, but next to useless in the assessment of project risk. The main source of AR4D project risk is not "price risk", i.e. not the risk that the that the value of the objective will change; but rather technical risk, i.e. the risk that the proposed technology will not function or scale as expected, due either to technical difficulties encountered in the research itself, or to unforeseen adversity in the enabling environment (e.g., government policy shifts, deregulation issues, etc.).

Even in the corporate ROV context, the replicating portfolios argument is met with considerable skepticism [@borison2005real]; and proponents of the argument acknowledge that its validity is limited to a narrow range of project types---which does not include R&D projects [@schwartz2013real]. There is, nonetheless, a certain methodological pressure to invoke the argument because it forms the premise of another argument known as the "no arbitrage rule", which in turn forms the premise of the Black-Scholes partial differential equation governing option pricing theory [@hull9thEdition]. 



The replicating portfolios argument also figures critically in the formal derivation of the Black-Scholes partial differential equation (PDE), which governs all option pricing formulae, not to mention all financial derivatives more broadly [@hull9thEdition]. However, in the process, it also leads to a celebrated corollary known as "risk-neutral valuation", which is problematic in the AR4D context. Risk-neutral valuation implies that If it is not possible to make risk-free profits above the risk-free rate of return (the “no-arbitrage” rule), then investors are risk-neutral, i.e. "investors do not increase the expected return they require from an investment in order to compensate for increased risk" [@hull9thEdition]. Clearly, AR4D donors are not risk neutral. They are willing to bet on a high risk project only if there is a sufficiently high expected return.
...
<!-- they require from an investment in order to compensate for increased risk” (Hull, 2015) -->
 ...
 In settings where the , some ROV practitioners invoke an alternative argument known as "complete markets" [@pennings1997option]. There is methodological pressure to adopt either of these arguments because they figure centrally in the formal derivation of the fundamental theory of options valuation

[Even in corporate context...complete markets..."all risks can be hedged" through the purchase/shorting of securities...Clearly, AR4D technical risks cannot be hedged in this manner.]

[Also, the issue of price risk vs. research risk, the latter being the important one in AR4D, and not reflected in markets and thus not captured in the default theoretical set up/default interpretation of the B-S PDE....  AR4D NPV must be estimated the "hard way"]

Adaptation of option pricing theory to the AR4D context must somehow be derived on a different basis, whereby project NPV is non-traded, and in a way that does not imply risk-neutral valuation. and that considers technical risk as opposed to market risk.
...
---whereas, in the AR4D context, project NPV must be estimated the "hard way", by ex-ante impact assessment.
...
Even in the corporate ROV context, the replicating portfolios argument faces considerable skepticism [@borison2005real]; and proponents of the argument acknowledge that its validity is limited to a narrow range of project types---which does not include R&D projects [@schwartz2013real]. Corporate R&D...complete markets [@pennings1997option]. There is methodological pressure to adopt either of these arguments because they figure centrally in the formal derivation of the fundamental theory of options valuation (i.e. the Black-Scholes partial differential equation).
[More problematically still, the replicating portfolios argument leads to a corollary known as "risk-neutral valuation", i.e. "investors do not increase the expected return they require from an investment in order to compensate for increased risk" [@hull9thEdition]. Clearly, AR4D donors are not risk neutral. They are willing to bet on a high risk project only if there is a sufficiently high expected return. ROV practitioners feel compelled to ].... Here we present an alternative derivation of the Black-Scholes PDE based only the assumption of normally distributed log returns, and that the present value of future impacts appreciates at the discount rate.
...
and we extend the framework to multi-stage projects.
