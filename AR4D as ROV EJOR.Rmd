...
\begin{equation}
ROV(0) = e^{-r T_0} E[\max(x(T_0) - K_0, 0)]|_{t = 0}
\label{eq:basic}
\end{equation}

Where $x(T_0)$ is project net present value (NPV) at the end of the research period $T_0$, $K_0$ is the cost of funding the subsequent project stage, and $r$ is the discount rate. By contrast, the comparable conventional appraisal of the project is calculated as
...
The ROV of stage 1 of a three stage project, $ROV_3(0)$, may be expressed as follows.

\begin{equation}
ROV_3(0) = e^{-r T_2} E[\max(ROV_2(T_2) - K_2, 0)]|_{t = 0}
\label{eq:eg3stage}
\end{equation}

Where $ROV_2(T_2)$ is the ROV of stage 2 evaluated at $T_2$, the end of stage 1, $K_2$ is the cost of investing in stage 2 upon successful completion of stage 1; and stage 1 is considered successfully completed if $ROV_2(T_2) > K_2$. $ROV_2(T_2)$, in turn, is defined

\begin{equation}
ROV_2(T_2) = e^{-r T_1} E[\max(ROV_1(T_1) - K_1, 0)]|_{t = T_2}
\end{equation}

Where $ROV_1(T_1)$ is the ROV of stage 3 evaluated at $T_1$, the end of stage 2, $K_1$ is the cost of investing in stage 3 upon successful completion of stage 2; and Stage 2 is considered successfully completed if $ROV_1(T_1) > K_1$. $ROV_1(T_1)$, finally, is just the one stage ROV of the project NPV evaluated at $T_1$.

\begin{equation}
ROV_1(T_1) = e^{-r T_0} E[\max(x(T_0) - K_0, 0)]|_{t = T_1}
\end{equation}

If $ROV_3(0)$ is greater than the investment $K_3$ required to implement stage 1, then investment in stage 1 is justified. The conventional CBA criterion would be

\begin{equation}
e^{-rT_0} E[x(T_0)]|_{t = 0} - \sum_{i = 0}^{2} e^{-r T_i} K_i > K_3 \:\: \rightarrow \:\: \text{invest in project}
\end{equation}
<!-- Generally, -->
<!-- \begin{equation} -->
...
# The incompatible underpinnings of existing ROV approaches

## Near market vs. far from market
...
## Modeling the time evolution of project NPV \label{sec:gBmDefens}
...
\begin{equation}
e^{-r T_0} E[\max(x(T_0) - K_0, 0)]\bigr|_{t = 0} = x(0) \Phi(\delta_0 + s \sqrt{T_0}) - e^{-r T_0} K_0 \Phi(\delta_0)
\label{eq:rov}
\end{equation}
...
The value of $\Phi(\delta_0)$ is instructive. It is the probability that $x(T_0)$ will exceed the exercise cost $K_0$. In real options parlance, it is the probability that the option finishes "in the money". To see this, note that $\delta_0$ may be rewritten as the negative standard score of $\ln(K_0/x(0))$.
...
&= 1 - \Prob \left( \frac{ \ln \left(\frac{K_0}{x(0)} > 0 \right) = \Prob \left( \frac{ \ln \left(\frac{K_0}{x(0)} < 0 \right)
\end{split}
\end{equation}
...
Note, moreover, that log changes in $f$ are normally distributed with mean and variance

\begin{equation}
E\left[\left( \frac{f(T_0)}{f(0)} \right) \right]\bigr|_{t = 0} = \left(E\left[ \frac{\Delta f}{f} \right] - \eta_{1,0}^2 \frac{s^2}{2} \right) T_0
\label{eq:muLf1}
\end{equation}

\begin{equation}
Var\left[\left( \frac{f(\tilde{T_0})}{f(0)} \right) \right]\bigr|_{t = 0} = \eta_{1,0}^2 s^2 T_0
\label{eq:sigLf1}
\end{equation}

(See Hull [-@hull9thEdition] for details.)

The function $f(x, t)$ in mind here is, of course, ROV. Black and Scholes [-@black1973valuation] famously noted that equations \ref{eq:gbmEq} and \ref{eq:itoLem} could be combined so as to eliminate the random term as follows.
...
Equating the right-hand side of this equation with the right-hand side of the previous equation, the $\Delta t$'s cancel, resulting in the Black-Scholes PDE.
...
And the mean log change in Equation \ref{eq:muLf1} may be rewritten 

\begin{equation}
E\left[\left( \frac{f(\tilde{T_0})}{f(0)} \right) \right]\bigr|_{t = 0} = \left(r - \eta_{1,0}^2 \frac{s^2}{2} \right) T_0
\label{eq:muLf1_b}
\end{equation}

While the Ito calculus is not necessary for deriving the single stage ROV formula (this was demonstrated in section \ref{sec:straightInt}), it becomes useful in the derivation of a formula for multi-stage ROV because it provides a clear demonstration that ROV itself is lognormally distributed with computable parameters given in Equations \ref{eq:muLf1_b} and \ref{eq:sigLf1}; and that this is recursively true for the ROV of $f$, and for the ROV of the ROV of $f$, ad infinitum, as detailed in the next section.

## Extension to multi-stage ROV \label{sec:method}

For the derivation of a multi-stage ROV formula, it is henceforth necessary to index ROV as $f_1$, $f_2$,..., on up to $f_n$ so as to represent the ROV of stage $n$,..., 2, 1, research, respectively. (The $f_i$ correspond to the $ROV_i$ appearing in the Introduction). Following this schema, $f_0$ corresponds to project NPV $x$.
...
\begin{equation}
\begin{split}
f_2(f_1, t; T_1, K_1, s, r)|_{t = 0} &= e^{-r T_2} E[\max(f(T_1) - K_1, 0)]\bigr|_{t = 0} \\
&= f_1(0) \Phi(\delta_1 + \eta_{1,0} s \sqrt{T_1}) - e^{-r T_2} K_1 \Phi(\delta_1)
\end{split}
\label{eq:rov2stage}
\end{equation}

Where $T_2$ is the time until completion of stage 1, $K_1$ is the research investment required upon successful completion of stage 1 at time $T_1$---where stage 1 is considered successfully completed if $f_1(T_1) > K_1$---and where
...
\begin{equation}
\Delta f_2 = \left( \frac{\partial f_2}{\partial f_1} r f_1 + \frac{\partial f_2}{\partial t} + \frac{s^2 f_1^2}{2} \frac{\partial^2 f_2}{\partial f_1^2} \right) \Delta t + f_2 \eta_{1,0} s \epsilon \sqrt{\Delta t}
\label{eq:itoLem2}
\end{equation}
...
\begin{equation}
E\left[ \Delta f_2 \right] = \left( \frac{\partial f_2}{\partial f_1} r f_1 + \frac{\partial f_2}{\partial t} + \frac{s^2 f_1^2}{2} \frac{\partial^2 f_2}{\partial f_1^2} \right) \Delta t
\label{eq:Ef2}
\end{equation}
...
As in the case of $f_1$, because $f_2$ is defined as an expected future value discounted back to the present, then the expected growth rate of $f_2$ is just $r \Delta t$. Equation \ref{eq:Ef2} may thus be set equal to $f_2 r \Delta t$, resulting in a Black-Scholes PDE for $f_2$.

\begin{equation}
rf_2 = f_1 r \frac{\partial f_2}{\partial f_1} + \frac{\partial f_2}{\partial t} + \eta_{1,0}^2 \frac{s^2}{2} f_1^2 \frac{\partial^2 f_2}{\partial f_1^2}
\end{equation}

Which may then be solved for $f_2$ under the boundary condition $f_2(0) = \max(f_1(T_1) - K_1, 0)$ as another way of obtaining Equation \ref{eq:rov2stage}. It follows, moreover, that $f_2$ is lognormally distributed with log return mean and variance

\begin{equation}
E\left[\left(\frac{f_2(T_1)}{f_2(0)} \right) \right]\bigr|_{t = 0} = \left(r - \eta_{2,0}^2 \frac{s^2}{2} \right) T_1
\end{equation}

\begin{equation}
Var\left[ \left( \frac{f_2(T_1)}{f_2(0)} \right) \right]\bigr|_{t = 0} = \eta_{2,0}^2 s^2 T_1
\end{equation}

Such that the same process can be repeated for a function $f_3(f_2, t)$ to obtain the ROV of stage 2 research of a 3 stage research project, and so on, for any ROV of stage 2 of an $n$-stage project, $f_n(f_{n - 1}, t)$,

\begin{equation}
\begin{split}
f_n(f_{n - 1}, t; T_{n - 1}, K_{n - 1}, s, r)|_{t = 0} &= e^{-r T_n} E[\max(f(T_{n - 1}) - K_{n - 1}, 0)]\bigr|_{t = 0} \\
&= f_{n - 1}(0) \Phi(\delta_{n - 1} + \eta_{n - 1, 0} s \sqrt{T_{n - 1}}) - e^{-r T_{n - 1}} K_{n - 1} \Phi(\delta_{n - 1}) \:\:;\:\:\: n \in \mathbb{N}_0
\end{split}
\label{eq:ROVn}
\end{equation}

Where $T_n < T_{n - 1} <...< T_0$ is the time until completion of stage 1 of the $n$-stage research project, $K_{n - 1}$ is the research investment required upon successful completion of stage 1 at time $T_{n - 1}$---where stage 1 is considered successfully completed if $f_{n - 1}(T_{n - 1}) > K_{n - 1}$---and where
...
\begin{equation}
\frac{\partial f_{n - 1}}{ \partial f_{n - 2}} = \Phi( \delta_{n - 2} + \eta_{n - 2, 0} s \sqrt{T_{n - 2}}) + \frac{\partial \eta{n - 2, 0}}{ \partial f_{n - 2}} f_{n - 2} \phi(\delta_{n - 2} + \eta_{n - 2, 0} s \sqrt{T_{n - 2}})
\end{equation}
...
The ROV of stage 2 of an $n$-stage project is thus calculated recursively, starting with the calculation of $f_1(x, t; T_0, K_0, s, r)|_{t = 0}$. In this process of 


$\Phi(\delta_{i - 1})$ is the probability that $f_{i - 1}$ will equal or exceed $K_{i - 1}$ by time $T_{i - 1}$, thus triggering investment in the subsequent stage. In financial parlance, it is the probability that $f_i$ will finish "in the money".

 and can thus be used as a measure of how senstitive the value of an investment in any stage of research is with respect to changes in the underlying project NPV. 

<!-- And where, for all $i$, the expected growth rate of $f_i$ is, by definition, $r \Delta t$, and, by Ito's lemma, the growth rate variance of $f_i$ is $\eta_{i, 0}^2 s^2 \Delta t$, and $f_i$ is lognormally distributed with log return mean and variance -->
<!-- \begin{equation} -->
...
In order to make the calculation, we require the research stage and final launch costs $K_0, K_1, \dots, K_4$ and time horizons $T_0, T_1, \dots, T_3$ listed in Table 1, as well as the quarterly discount rate $r$, the project NPV volatility $s$, and the project NPV at the time of the go/no-go decision to invest in stage 1 LBr research, $x(0)$.
...
In cases where no ex-ante impact assessment of project NPV is available, it may be instructive to solve stage 1 ROV for the break even project NPV, i.e. the value of $x(0)$ required to yield a stage 1 ROV equal to the stage 1 cost. In this example, the stage 1 break even project NPV is \$`r round(X0min * 10^-6, 3)` million. Any project NPV above this value results in a stage 1 ROV greater than the investment required to implement stage 1.
