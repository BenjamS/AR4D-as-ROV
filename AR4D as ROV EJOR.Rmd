...
## Derivation via the stochastic calculus and Relation to the Black-Scholes derivation
...
For the derivation of a multi-stage ROV formula, it is henceforth necessary to index $f$ as $f_1$, $f_2$,..., on up to $f_n$. (The $f_i$ correspond to the $ROV_i$ appearing in the Introduction). Following this schema, $f_1$ corresponds to $f$ above, and $f_0$ corresponds to project NPV $x$.
...
\begin{equation}
E\left[\left(\frac{f_2(T_2)}{f_2(0)} \right) \right]\biggr|_{t = 0} = \left(r - \eta_{2, 0}^2 \frac{s^2}{2} \right) T_2
\end{equation}

\begin{equation}
Var\left[ \left( \frac{f_2(T_2)}{f_2(0)} \right) \right]\biggr|_{t = 0} = \eta_{2,0}^2 s^2 T_2
\end{equation}
