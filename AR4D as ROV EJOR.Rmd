...
In the method section farther below, we show how an AR4D compatible Black-Scholes PDF can be derived from the definition of project NPV, and how this leads to a risk adjusted discounting corollary instead of risk neutral valuation. But first it is necessary to disassociate the present approach from the corporate ROV literature on another front.
...
In the AR4D context, the grounds for swallowing the bitter pill of numerical methods are especially narrow. Firstly, it is not even clear what a realistic time evolution of AR4D project NPV looks like. The time and expense involved in AR4D ex-ante impact assessment make frequent, regular updates of project NPV impractical, which in turn means that direct empirical inspection of the stochastic character of project NPV over a series of small, regularly spaced time steps is generally not possible---certainly not in the way that is possible for a price series.

In the corporate context, P&L attempt to resolve this issue by assembling from scratch an historical time series of project NPV; and then selecting and parameterizing their stochastic model based on this. Such an approach, arguably, does not so much resolve the issue as it does replace it with another set of problems. First and foremost, the historical project NPV must be carefully selected so as to be representative of the NPV trajectory of the new project under evaluation. Apart from a passing verbal assurance, P&L offer little indication that their assembled time series may be plausibly construed as representative of the stochastic character of the new project under evaluation []. In the AR4D context, projects are so heterogeneous in their scale, scope, aims, target populations and environments, and so forth, that finding a past project NPV trajectory that plausibly maps onto a present project's NPV amounts to a prohibitive challenge.

Secondly, even if this were possible, the practitioner then runs into small sample problems. In both the corporate and AR4D contexts, the number of substantive adjustments to project NPV over the project's lifespan tends to be small. To generate "enough" observations, P&L merge several past project NPV trajectories together into a single time series---which, of course, severely compounds the first problem. Even then, the authors come up with just 17 observations over a 5 year period []. Is this enough to select and parameterize a stochastic model of project NPV?

Thirdly, there is the subtle and somewhat philosophical sounding, but deeply consequential, issue of sampling rate. When does a change in project NPV occur? Does it occur when the relevant, causal event or set of events occurs, or only when management learns of its occurrence and announces the corresponding adjustment to NPV through reporting protocols? What about events that effectively change NPV, but that go unnoticed by management? P&L tacitly presume that changes in NPV occur only if and when management learns of their occurrence. This is understandable in the corporate context, where the value of company assets and activities is ultimately defined by the stock market, and hence (in principle) by publicly available information.

In the AR4D context, on the other hand, we are interested in characterizing the stochastic progression of real changes to project NPV, regardless of when or even if management or anyone else discovers and documents these changes. In this setting, then, one may reasonably conceptualize the project NPV trajectory as a series of negligibly small changes interspersed by relatively few large changes. 

Finally, for ROV purposes it is not even necessary... only the size distribution, not the specific time or even order in which they occur.

after all this trouble, only a 2% difference
not predicting here, only size distrubution matters for purposes of ROV
reporting protocols are non-random, problematic for cPp.


events affecting the NPV of projects within a general research category.  From a statistical perspective, such an approach arguably creates more problems than it solves. First of all, it requires the existence of an historical NPV time series that . P&L     changes just say events over the past 5 years.           Any attempt to painstakingly assemble from scratch a dataset of project NPV time series would arguably creates more problems than it solves.  Corporate ROV practitioners who do not invoke replicating portfolios face the same problem. P&L go to the extraordinary length of painstakingly building a dataset from scratch by documenting the NPV time series of several past projects; and then specifying and estimating a stochastic model for current project NPV on this basis.    How, then, to justify one stochastic model as more realistic than another?

P&L [] propose the extraordinary alternative of  While it may be possible to introduce an AR4D reporting protocol by which project NPV is periodically revisited and updated based on research progress and changes in relevant external factors (e.g. government policy shifts), and thus to generate an historical record of such time series, it is doubtful whether such an approach offers any insight into the stochastic character of any current AR4D project NPV. Sampling error and controlling for changes over time. Even if achieved, extrapolation to the present is further complicated by small sample size issues.    Series guaranteed to look like a cPp, but this may be merely due to the sampling rate, not to the stochastic character of project NPV. , cPp is valid only if the interval size between changes is random. The non-random, periodic character of internal reporting       Their project NPV time series resemble the cPp series in the right panel of Figure. The problems that arise in its stead are arguably greater than the one that is eluded. sampling error in the selection of past project NPV, not controling for changes in basic circumstances (funding, strategic objecties, instututional change, etc.), even if achieved...low data ..impractical/not scalable in terms of time and cost, and ...Less obviously,  Assuming that a sufficient number of ex-ante impact assessments are conducted, the observed stochastic character is due to the sampling rate, not necessarily of project NPV. Moreover 
...
the difference between gBm and the methodoligically much more complex cPp is a matter of sampling rate, i.e. time step size. Tankov shows how cPp can be arbitrarily approximated by parameterization of gBm. ...so long as no change can be substituted by negligibly small change.



Efforts to do so generate effectively replace one problem with another. Pennings and Lint, for example, did empirically inspect, but this generated selection bias... Instead, the stochastic character of project NPV must be inferred via deductive reasoning.


 

As mentioned above, in the AR4D context (and, we suspect, many corporate contexts), project NPV may not be modeled as a function of prices, but rather by ex-ante impact assessment.  changes in project NPV occur as a result of research and non-research related events, and it is reasonable to assume that, generally speaking, the time evolution of project NPV is characterized by relatively few large shifts---occurring, e.g., at the critical test points in the project timeline and/or major events in the enabling environment, such as elections---interspersed with relatively many small shifts in NPV due to non-research related fluctuations in the enabling environment. Based on this rationale, the lognormal distribution is a reasonable choice as model of the size distribution of changes in project NPV.   ...the gBm parameters may be adjusted so as to approach a stochastic jump diffusion process arbitrarily closely.


The grounds for swallowing this bitter pill are arguably quite narrow in the AR4D context, where direct, empirical observation of the time evolution of project NPV is generally impossible, and thus direct inspection of its stochastic character is likewise impossible. cannot be indirectly observed via replicating portfolios.   project NPV cannot be modeled as a function of prices (i.e., where replicating portfolios may not be invoked), and where, consequently, there is no time series data by which to empirically inspect the stochastic character of project NPV. Pennings and Lint painstakingly construct a...jump process (cPp)....Only a 2% difference [].... by adjusting parameters the gBm can approximate a jump process...Tankov notes this []...
