...
The notion of a real option is born of the analogous concept of a European call option in financial contexts. The holder of a European call option on an underlying security (a stock, for example) has the right, but not the obligation, to purchase the security for a predetermined price (called the "strike" or "exercise" price) at the end of a certain time period.
<!-- [If the security price is higher than the strike price at the end of the time period, then it is profitable for the investor to exercise their right to purchase the security at the strike price. Otherwise, the ...] -->

Analogously, donors who "buy a real option" commit to funding a real project for a certain time period, at the end of which they have the right, but not the obligation, to continue funding the subsequent stage of the project. The project's net present value (NPV) is analogous to the price of the underlying security in the financial context, while the cost of the subsequent stage is analogous to the exercise price. (The cost of the subsequent stage must be known up front for the analogy to hold.) For a detailed introduction to real option value, see Trigeorgis [-@trigeorgis1993real] and McGrath [-@mcgrath2000assessing].

The methodological details of real option valuation can vary considerably from one applied context to another. But, generally speaking, project real option value (ROV) is often higher than project NPV, especially for projects with high risk and a high expected reward, such that a risky project that would normally be rejected due to low or negative NPV might pass muster on an ROV basis.
...
...Also adaptation of something called "risk-neutral valuation" to the real options context. Financial option valuation depends upon something called "risk-neutral valuation", which in turn derives from something called the "no arbitrage rule", which is only valid in a competitive market setting. which has no clear analogue in the non-market real option setting. Many, if not most, real options contexts are non-market contexts. AR4D projects typically address market failures. Here we show an alternative way to establish the same premise that is valid for non-market settings.

In the AR4D context, real option contracts are, in principle, mutually beneficial for both donors and researchers alike. From the donor's perspective, a real option contract is a way to tentatively lay claim to promising, but distant and uncertain, research results, while at the same time guaranteeing an escape route in the eventuality that the project underperforms. The resulting fiscal agility then translates into longer term funding commitments, which is music to the ears of the research community.

It is worth noting, moreover, that donors already acquire option value in their research investments---even if they do not explicitly name it as such---since they effectively reserve and exercise the right to discontinue funding at specific time intervals. (Typical AR4D funding arrangements nowadays last for no more than 1-5 years, at the end of which the contract is up for review and renewal/cancellation.) In this sense, ROV merely formalizes an already existing practice, and more accurately quantifies the real value acquired by the donor in exchange for their investment. By the same token, default methods of project appraisal that neglect to price in the donor's option value (cost-benefit analysis, for example) effectively understate the real value acquired by the donor.

ROV has been applied and discussed in private sector research contexts [@mitchell1990alternative; @pennings1997option; @cassimon2004valuation; @doctor2001managing; @newton2004real]. Here we extend this treatment to the AR4D context. 
The ROV context differs from the financial context in two key respects, which complicate adaptation of financial option valuation tools to the real options context. First of all, 

Adaptation of financial option evaluation to the real options context is complicated by a number of key respects  lack of data by which to calculate project NPV volatility, 
