...
In other words, $m \Delta t$ and $s^2 \Delta t$ are the mean and variance of the instantaneous arithmetic change. The expected value of project NPV at some future time $\tilde{t}$ ($t < \tilde{t} < T$) as evaluated at time $t$ is given by
...
And the mean and variance of the log change over any finite interval $\tau$ is given by
...
The ROV equation follows by evaluating Equation \ref{eq:basic} through straightforward integration.

\begin{equation}
e^{-r\tau} E[\max(x(\tilde{t}) - K, 0)]\bigr|_{t = \hat{t}} = e^{-\left(r-\frac{s^2}{2}\right) \tau} x(\hat{t}) \Phi(\delta + s \sqrt{\tau}) - e^{-r \tau} K \Phi(\delta)
\label{eq:rov}
\end{equation}
...
\begin{equation}
\delta = \frac{1}{s \sqrt{\tau}} \ln \left(\frac{x(\hat{t})}{K} \right)
\end{equation}
...
Because $x(t)$ is a gBm, then, By Ito's lemma, the evolution of a function $f(x, t)$ is described as follows.
...
In this article, $f$ is the option value of $x$. Black and Scholes [-@black1973valuation] famously noted that equations \ref{eq:gbmEq} and \ref{eq:itoLem} could be combined so as to eliminate the random term as follows.
...
Black and Scholes then go on to solve this PDE for $f$ under the boundary condition $f(T) = \max(x(T) - K, 0)$, resulting in their famous expression for financial option value, stated here for convenience.
...
The AR4D ROV expression may be derived **more** Black-Scholes by replacing the logic behind Equation \ref{eq:BSarb}, which does not apply in the AR4D context, with the following logic:
...
expression      Equation \ref{eq:BSarb} does not apply. Instead, 
