...
To characterize this as the conventional "NPV approach", as is sometimes done in the ROV literature [e.g., @koppl2013real], can be confusing since $x(t)$ is already defined as project NPV at any time $t$ (net of $K_0$), and appears in the calculation of ROV. To be clear, then, in the present article, the conventional manner of project appraisal in Equation \ref{eq:conv} is referred to as the "Cost Benefit Analysis (CBA) approach" or "CBA basis"; whereas "project NPV", denoted $x(t)$, refers to the expected value at time $t$ of the net benefit stream resulting from release and outscaling of the research product after time $T_0$, net of any costs incurred at or prior to time $T_0$.
...
The function $f(x, t)$ in mind here is, of course, ROV. Because ROV is defined as an expected future value discounted back to the present (Equation \ref{eq:basic}), then its expected instantaneous growth rate is, by definition, $E[\Delta f / f] = r \Delta t$. Equation \ref{eq:Ef1} may thus be set equal to $r \Delta t$, resulting in the following PDE.
...
This may then be solved for $f$ under the boundary condition $f(T_0) = \max(x(T_0) - K_0, 0)$, resulting in the same expression obtained through straightforward integration in Equation \ref{eq:rov}. (See Appendix for details.)
...
Equating the right-hand side of this equation with the right-hand side of the previous equation, the $\Delta t$'s cancel, resulting in Equation \ref{eq:bsPDEar4d}.
<!-- the Black-Scholes PDE. -->
<!-- \begin{equation} -->
<!-- \left(f - \frac{\partial f}{\partial x} x \right) r = \frac{\partial f}{\partial t} + \frac{s^2 x^2}{2} \frac{\partial^2 f}{\partial x^2} -->
<!-- \end{equation} -->

Note that the Black-Scholes derivation hinges critically upon the assumption that $x(t)$ follows a gBm. As discussed above in section \ref{sec:gBmDefens}, this assumption is largely rejected in the literature subsequent to Black and Scholes' seminal paper, thus rendering the closed form solution in Equation \ref{eq:rov} of mere academic interest in the financial and corporate ROV contexts.
...
While the Ito calculus is not necessary for deriving the single stage ROV formula (this was demonstrated in section \ref{sec:straightInt}), it becomes useful in the derivation of a formula for multistage ROV because it provides a clear demonstration that ROV itself is lognormally distributed with computable parameters given in Equations \ref{eq:muLf1_b} and \ref{eq:sigLf1}; and that this is recursively true for the ROV of $f$, and for the ROV of the ROV of $f$, and so on, ad infinitum, as detailed in the next section.

## Extension to multistage ROV \label{sec:method}

For the derivation of a multistage ROV formula, it is henceforth necessary to index $f$ as $f_1$, $f_2$,..., on up to $f_n$. (The $f_i$ correspond to the $ROV_i$ appearing in the Introduction). Following this schema, $f_1$ corresponds to $f$ above, and $f_0$ corresponds to project NPV $x$.

Because $f_1$ is itself lognormally distributed, the same formula used to evaluate the ROV of $x(T_0)$ can be used to evaluate the ROV of $f_1(T_1)$. That is to say, Equation \ref{eq:genEq} may be evaluated at $q = f_1(T_1)$ and $C = K_1$ to calculate $f_2$, the value of the option to invest in the last stage of research.
...
Whereby it follows that the same formula used to evaluate the ROV of $x(T_0)$ and $f_1(T_1)$ can be used to evaluate the ROV of $f_2(T_2)$. That is to say, Equation \ref{eq:genEq} may be evaluated at $q = f_2(T_2)$ and $C = K_2$ to calculate $f_3$, the value of the option to invest in the second to last stage of research. 
...
Such that a general formula for the ROV of stage 1 of any $n$-stage project may be resolved by evaluating Equation \ref{eq:genEq} at $q = f_{n - 1}(T_{n - 1})$, $C = K_{n - 1}$, giving
...
In this process, the respective probabilities of finishing in the money (project stage success), $\Phi(\delta_i)$, and sensitivity to changes in project NPV, $\eta_{i, 0}$, are calculated for each stage. To be clear, the quantity $\eta_{i, 0}$ is the elasticity of $f_i$ with respect to project NPV for each project stage $i \in [0, n]$, defined

\begin{equation}
\eta_{i, 0} = \frac{f_{i - 1}}{f_{i}} \frac{\partial f_{i}}{\partial f_{i - 1}} \eta_{i - 1, 0} \:\:;\:\:\: 0 \geq i \leq n \:\:\: \eta_{0,0} = 1
\end{equation}

As such, this is an instructive measure of ROV sensitivity to changes in project NPV at each project stage. The calculation of $\eta_{n - 1, 0}$ appearing in Equation \ref{eq:ROVn} requires calculation of $\partial f_{n - 1} / \partial f_{n - 2}$, which works out as follows.
...
Which may then be solved for $f_n$ under the boundary condition $f_n(T_n) = \max(f_{n - 1}(T_{n - 1}) - K_{n - 1}, 0)$ as another way of obtaining Equation \ref{eq:ROVn}.
...
<!-- \appendix -->
<!-- \section{Proof of equation \ref{eq:genEq} through straightforward integration} -->
...
The definition in equation \ref{eq:def} resolves as follows.
...
Which is equivalent to Equation \ref{eq:rov} when $t = 0$, $T = T_0$, $K = K_0$.
