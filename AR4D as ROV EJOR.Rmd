...
A project's real option value (ROV) is often higher than its NPV, especially in the case of high risk, high reward projects, such that a project that would normally be rejected in a conventional cost-benefit analysis might pass muster on an ROV basis. For a detailed introduction to ROV, see Trigeorgis [-@trigeorgis1999real]. For an introduction to ROV in R&D contexts specifically, see [].
...
Donors invest in each of these stages one at a time, consecutively, with investment in the subsequent stage contingent upon successful completion of the current stage. The donor's investment in stage one of a three stage project, then, is analogous to purchasing an option on stage two---i.e., the right but not the obligation to invest in stage two---which is turn an option on stage three. Existing real option methods, meanwhile, focus primarily on single stage projects. The one exception is a multistage model proposed by Cassimon et al. [-@cassimon2004valuation] for the evaluation of pharmaceutical trials, building upon the two stage or "compound option" model originally developed by Geske in the financial context [-@geske1979valuation]. Again, such work is not readily extensible to AR4D due to its corporate and financial theoretical underpinnings.

In this article, we derive a multistage ROV model for the AR4D project context. Rather than build upon existing corporate ROV methods, we demonstrate why the financial theoretical underpinnings of such models are inapplicable in the AR4D context, and develop suitable foundations in their place. Our critique of corporate ROV is found in the literature review, immediately below. This includes treatment of the choice of stochastic model to represent the time evolution of project NPV. Whereas corporate ROV methods generally follow the financial trend of specifying highly realistic, but also methodologically costly, jump diffusion models of project NPV, here we argue that the much simpler geometric Brownian motion (gBm) model strikes the right balance between realism and tractability for the AR4D context.
<!-- The first step in this work is a studious avoidance of the theoretical underpinnings of corporate ROV methods, and the development of suitable foundations in their place. -->

Remarkably, the foundations for AR4D project ROV developed to replace the critiqued financial foundations require no set of substitute assumptions, but rather follow simply from the definition of project NPV. Specifically, they follow from the fact that NPV, by definition, appreciates at the discount rate. From this, we show, it follows that AR4D donors are not risk neutral, but rather discount future values proportional to their risk. It also follows that ...multistage remarkably simple and tractable compared to the model developed by Cassimon et al. [-@cassimon2004valuation].

In the discussion, we briefly demonstrate some useful extensions and modifications to the ROV model. In particular, we show how the model can be modified to 1) include "abandonment value", i.e., the value often generated by AR4D projects in the form of human and physical capital upgrades, regardless of the success or failure of the main research activities; 2) account for the scaling relation between investment and impact.

From the outset it is important to distinguish the notion of ROV pursued here (Equation \ref{eq:basic}) and originally developed by Meyer [], from a so-called "real option approach" already followed in numerous agricultural R&D appraisal studies []. The purpose of the ROV approach developed here is to appraise, from the donor's perspective, AR4D projects with known time horizons in a way that prices in option value. The other "real option approach" currently implemented in the agricultural R&D literature is, more accurately, an "optimal delay approach", in the sense that it is primarily concerned with determining the optimal delay, from a social planner's perspective, of the release and outscaling of a finished agricultural R&D product with uncertain environmental impact []. That is to say, it is not primarily concerned with the valuation of the R&D product itself, which is what we pursue here.
...
-This is followed by an extension to the multistage setting.
...
<!-- The relevant literature is engaged throughout the article as needed. While AR4D is the motivating context for the present work, the $n$-fold ROV model derived here is broadly applicable to any multistage, far-from-market, real option context. -->
...
The application of real option valuation (ROV) to the AR4D context is complicated by the multistage structure of most AR4D projects. 
...
## A studious avoidance of financial/corporate ROV theoretical foundations
...
\begin{equation}
e^{-r\tau} E[\max(x(T) - K, 0)]\bigr|_{t = \hat{t}} = e^{-\left(r-\frac{s^2}{2}\right) \tau} x(\hat{t}) \Phi(\delta + s \sqrt{\tau}) - e^{-\frac{s^2}{2} \tau} K \Phi(\delta)
\label{eq:rov}
\end{equation}
...
\begin{equation}
\delta = \frac{\ln \left(\frac{x(\hat{t})}{K} \right)}{s \sqrt{\tau}}
\end{equation}
