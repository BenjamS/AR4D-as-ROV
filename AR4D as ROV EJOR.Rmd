...
In this article, we develop an ROV method in the AR4D context. Up until now, ROV methods have developed in the context of corporate projects. A simple extension of this work to the AR4D context is not possible due to the non-market nature of AR4D. The first step in the present work is to dismiss the theoretical premises of existing ROV methods, and to develop suitable foundations in their place.

The task before us is also complicated by the multi-stage structure of most AR4D projects. For example, many AR4D plant breeding projects involve a preliminary greenhouse and/or field trial(s) stage, followed by more advanced, multi-location field trials [@covarrubias2022breeding]. In transgenic projects, additional toxicological trials are required, followed by the compilation of a regulatory dossier and application for deregulation in the target countries where the new variety is to be released. Finally, there is the release, distribution, and scaling stage, which involves careful orchestration of complex networks of public-private partnerships. Each stage in the series is contingent upon successful completion of the previous stage.

The ROV literature focuses primarily on single stage projects. In the pharmaceutical context, Cassimon et al. [-@cassimon2004valuation] extend corporate ROV to multi-stage projects, building on the "compound option" model developed by Geske in the financial context [-@geske1979valuation]. This work is not readily extensible to AR4D due to its corporate and financial theoretical underpinnings. Here we develop a multi-stage ROV model on the suitable foundations laid in the first step.
...
In due course, we also introduce two relatively minor adjustments to account for abandonment value, and scaling, which existing ROV models omit.  ...and scaling of last stage with investment...
