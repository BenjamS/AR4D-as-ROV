...
The LBr potato project is structured in four consecutive stages. These stages and their associated costs and time durations are summarized in Table x, based on figures documented by Schiek et al. [-@schiek2016demys]. This is a purely pedagogical exercise designed to illustrate the use of the multi-stage ROV model derived in the previous sections. While the exercise is based on real project stage costs and time horizons, the other parameter values are hypothetical. The resulting calculations should therefore not be interpreted as an authoritative valuation of LBr potato research.
...
In order to make the calculation, we require the research stage and final launch costs $K_0, K_1, \dots, K_4$ and time horizons $T_0, T_1, \dots, T_4$ listed in Table x, as well as the quarterly discount rate $r$, the project NPV volatility $s$, and the project NPV at the time of the go/no-go decision to invest in stage 1 LBr research, $x(0)$.
...
Project volatility $s$ is set to `r round(s, 3)`, which corresponds to a coefficient of variation of `r cv`, reflecting the high uncertainty that typically surrounds the expected impact of a disruptive new technology a decade or more in the future.

```{r, fig.show = "hold", fig.width = 4, fig.height=3, fig.align="left", fig.cap="\\label{fig:netBens}(Top) A hypothetical 12 year projection of discounted and undiscounted net benefits due to release of LBr potato in the target countries. (Bottom) The integral of discounted net benefits up to each year.", echo = FALSE}
# Define functions
...
At the 2015 go/no-go decision point, no published ex-ante impact assessments of LBr potato in the target countries exist. For the pedagogical purposes of this exercise, to calculate $x(0)$ we assume the net benefit projection in Figure \ref{fig:netBens}, top panel. More specifically, we assume that annual net benefits ($y(t)$) increase by `alphaYrly_discrete * 100`$\%$ year on year due to increasing adoption in the target countries; and reach $\$$ `dNVT0ref * 10^6` in year `tRef / 4` after release. This may be expressed
...
Where, for the quarterly time step in Figure \ref{fig:netBens}, $\hat{t} =$ `r tRef`, $y(\hat{t}) =$ `dNVT0ref * 10^6`, and $\alpha = $ `alphaQrly`.

Continuously discounted net benefits ($e^{-rt} y(t)$) are overlaid onto the undiscounted net benefit projection in the top panel of Figure \ref{fig:netBens}. Integration of the discounted net benefits with respect to $t$ over $[T_0, T_{-1}]$ gives project NPV at time of release ($x(T_0)$), presented in the bottom panel of Figure \ref{fig:netBens}.

\begin{equation}
\begin{split}
x(T_0)|_t &= \int_{T_0}^{T_{-1}} e^{-rt} y(t) \: dt \\
&= \frac{y(\hat{t})}{\hat{t}^{\alpha} r^{\alpha + 1}} ( -\Gamma(\alpha + 1, rt) + \Gamma(\alpha + 1, 0) )
\end{split}
\end{equation}

The upper bound $T_{-1}$ is a net benefit cutoff set by the donor. Net benefits continue beyond $T_{-1}$, but the donor is only interested in net benefits up to $T_{-1}$. In this exercise, the donor sets $T_{-1}$ to `r 4 * nbCutoffYr` quarters (`r nbCutoffYr` years) after release. This gives a project NPV of $\$$ `r NPVT0[nbCutoffQtr]` million at time of release $T_0$, i.e. at the end of stage 4 research. The project NPV discounted back to the time of the donor's go/no-go decision to fund stage 1 research is then $x(0) = e^{-rT_{4}} x(T_0) = \$$`r round(X0 * 10^-6, 3)` million.

The upscaling cost is unknown. For present purposes we set it to $\$$`r Kvec[1] * 10^-5` hundred thousand.

We now have everything we need. Stage 1 research ROV is calculated by first calculating the ROV of stage 4, the last stage, and then calculating the ROV of stage 3, and so forth, recursively, as explained in section section \ref{sec:method}. In the present exercise, this results in a stage 1 ROV of $\$$`r f_n`, which exceeds the stage 1 cost of $\$$ `r stage1Invest` by $\$$`r f_n - stage1Invest`. On the ROV basis, then, the stage 1 investment is justified. Conventional CBA, meanwhile, gives a present value of $-\$$`r abs(CBA)`, whereby the stage 1 investment would be rejected (Equation \ref{eq:convCBA}).

\begin{equation}
NPV = x(0) - \sum_{i = 1}^4 e^{-r Ti} K_i - 530000 = -16779
\label{eq:convCBA}
\end{equation}

The recursively calculated ROVs for each stage of research are presented in Table .... Also included in this table are each stage's ROV elasticity with respect to project NPV and probability of success, which are calculated in the process of calculating ROV. This provides additional information may figure in the donor's decision making process. For example, the donor may be encouraged to see
...
uncertain input values can be varied (upscaling cost)
