...
Where $ROV_2(T_2)$ is the ROV of stage 3 evaluated at $T_2$ (the end of stage 1), $K_2$ is the cost of investing in stage 2 upon successful completion of stage 1; and stage 1 is considered successfully completed if $ROV_2(T_2) > K_2$. And $ROV_2(T_2)$, in turn, is defined

\begin{equation}
ROV_2(T_2) = e^{-r T_1} E[\max(ROV_1(T_1) - K_1, 0)]|_{t = T_2}
\end{equation}

Where $ROV_1(T_1)$, finally, is the ROV of project NPV ($x(T_0)$) evaluated at $T_1$ (the end of stage 2), defined in Equation \ref{eq:basic}. $K_1$ is the cost of investing in stage 3 upon successful completion of stage 2; and stage 2 is considered successfully completed if $ROV_1(T_1) > K_1$.
...
Because $f_1$ is itself lognormally distributed, the same formula used to evaluate the ROV of $x(T_0)$ can be used to evaluate the ROV of $f_1(T_1)$. That is to say, Equation \ref{eq:genEq} may be evaluated at $q = f_1(T_1)$ and $C = K_1$ to calculate $f_2$, the ROV of stage 2 of a 2-stage project.
...
Whereby it follows that the same formula used to evaluate the ROV of $x(T_0)$ and $f_1(T_1)$ can be used to evaluate the ROV of $f_2(T_2)$. That is to say, Equation \ref{eq:genEq} may be evaluated at $q = f_2(T_2)$ and $C = K_2$ to calculate $f_3$, the ROV of stage 2 of a 3-stage project.
...
And so on and so forth, it follows generally that the ROV of stage 2 of any $n$-stage project, $f_n$, is an Ito process,
...
Such that a general formula for the ROV of stage 2 of any $n$-stage project may be resolved by evaluating Equation \ref{eq:genEq} at $q = f_{n - 1}(T_{n - 1})$, $C = K_{n - 1}$, giving
...
The ROV of stage 2 of any $n$-stage project is thus calculated recursively, starting with the calculation of the last stage $f_1(x, t; T_0, K_0, s, r)|_{t = 0}$, which then feeds into the calculation of the second to last stage, $f_2(f_1, t; T_1, K_1, s, r)|_{t = 0}$, and so forth until the calculation of first stage $f_n(f_{n - 1}, t; T_{n - 1}, K_{n - 1}, s, r)|_{t = 0}$. The value $f_i$ calculated at each stage is the ROV of the subsequent stage $f_{i-1}$.

In this process, the respective probabilities of finishing in the money, $\Phi(\delta_i)$, and sensitivity to changes in project NPV, $\eta_{i, 0}$, are calculated for each stage. To be clear, the quantity $\eta_{i, 0}$ is the elasticity of $f_i$ with respect to project NPV for each project stage $i \in [0, n]$, defined
...
colnames(dfROVn) <- c("Stage i", "ROV of stage i+1", "ROV elasticity w.r.t. project NPV", "Probability of success")
dfROVn %>%
  knitr::kable(
    format = "latex",
    row.names = F,
