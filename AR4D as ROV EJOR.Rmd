...
Project NPV $x(t)$ is defined as post-R&D project net benefits discounted back to the release of the research product at time $T$. Time $t < T$ is the time of evaluation. Assuming $x(t)$ follows a gBm, then its evolution $\Delta x$ over a small time increment $\Delta t$ is described by
...
In other words, $r \Delta t$ and $s^2 \Delta t$ are the mean and variance of the instantaneous arithmetic change. The expected value of project NPV at some future time $\tilde{t}$ ($t < \tilde{t} < T$) as evaluated at time $t$ is given by

\begin{equation}
E[x(\tilde{t})]\bigr_{t} = e^{m \tau} x(t) \:\:;\:\:\: (\tau = \tilde{t} - t)
\end{equation}
...
\begin{equation}
\begin{split}
E \left[ \ln \left(\frac{x(\tilde{t})}{x(t)} \right) \right]  &= \left(r - \frac{s^2}{2} \right) \tau \\
Var \left[ \ln \left(\frac{x(\tilde{t})}{x(t)} \right) \right] &= s^2 \tau
\end{split}
\end{equation}

(See Hull [-@hull9thEdition] for details.)

Now, by definition, $m = 0$ and $E[x(\tilde{t})]\bigr|_t = x(t)$. That is to say, the project ex-ante impact assessment is not expected to change, as all such expected changes are already, by definition, included in the assessment. This is true even if one knows there is a good possibility that future assessments will differ from the present assessment, especially as more information becomes available. The "good possibility" of future assessments differing from the present assessment is quantified in the volatility parameter $s$.
...
\begin{equation}
E \left[ \ln \left(\frac{x(\tilde{t})}{x(t)} \right) \right]  = 0 \left(r - \frac{s^2}{2} \right) \tau\label{eq:ExT}
\end{equation}
