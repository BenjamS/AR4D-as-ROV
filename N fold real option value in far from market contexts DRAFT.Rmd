...
abstract: |
  Agricultural research for development scientists are increasingly required to valuate the impacts of their research projects. This is made difficult by the long time horizons and multi-stage, high risk, high reward, far-from-market nature of such projects. Research donors and scientists alike have expressed exasperation with the crudeness of the conventional cost-benefit analysis approach to such tasks, contributing to growing toxicity in donor-researcher relations. To alleviate the tension, here I adapt real options valuation to multi-stage, far-from-market, high risk, high reward projects (such as agricultural research for development projects). After a brief introduction to the basics of real options valuation, including an extension to projects with abandonment value, I proceed to derive a novel real options valuation model for far-from-market, multi-stage projects. To prepare the ground for this derivation, first I carefully adapt the key assumption of lognormally distributed returns to project net present value to the agricultural research for development context. Second, I argue that the assumption of risk-neutral valuation is inappropriate in far-from-market contexts; and demonstrate the mathematical implications of non-risk-neutrality for real options valuation. Based on this groundwork, I then derive the multi-stage or "$n$-fold", far-from-market real option value model. Finally, as an illustrative example, I use the derived model to valuate a real, 4-stage potato research project.
...
```{r Fig1, fig.show = "hold", fig.width = 4, fig.height=4, fig.align="center", fig.cap="\\label{fig:rovIllust}A hypothetical project's real option value and net present value plotted together over a range of moneyness values. The black line marks 0, while the dotted red line marks the sunk cost $S$ beneath which the project is rejected. The multiple ROV plots correspond to different levels of uncertainty (s). The higher the uncertainty, the greater the difference between ROV and NPV.", echo = FALSE}
...
This is the value of the option, but not the obligation, to disburse the funds $K_1$ required to implement the second stage of the 2-stage project, once the first stage is finished. Note that $K_1$ corresponds to the stage $n$ sunk cost $S$ in equation \ref{eq:rovBrawCond}. The time horizon $T_2$ refers to the duration of stage 1, and $B_1$ is the abandonment value, if any, generated during stage 1.
...
Where $T_3$ is the duration of the first stage, $B_3$ is any abandonment value associated with this stage, and $K_2$ the cost of implementing the second stage, of the 3-stage project.
...
table_caption <- "Note: Stage 1-4 costs and durations based on figures reported by Schiek et al. (2016). The launch cost and duration is hypothetical."
...
Based on these parameter values, the 4-fold option value of the LBr potato project works out to $\$$`r ROV` USD, which exceeds the stage 1 implementation cost ($\$$`r Kvec[n]`) by $\$$`r dif`. Hence the investment is justified under the parameter settings defined above. The default approach (expression \ref{eq:npvIneqN2}), by contrast, works out to $\$$`r conv`, which falls below the stage 1 cost by $\$$`r -dif_conv`. On the default basis, then, the LBr project would be rejected.
<!-- (equation \ref{eq:rovBn}) -->

The ROV approach provides additional output that may be of use in decision making. The probability of stage 1 research expiring in the money ($\Phi(\delta_4)$) is an encouraging `r Phi2`, while the elasticity of the 4-fold ROV with respect to project NPV is a moderate `r etaN0`, meaning that a $1\%$ change in project NPV results in a `r etaN0`$\%$ change in the underlying. This indicates considerable sensitivity to changes in project NPV.
...
The exercise so far assumes that abandonment value is zero. Now consider a stage 1 abandonment value of $\$$`r Bvec[n]`, corresponding to the value of permanent upgrades to human and fixed capital at the national agricultural research institutions of the target countries. Then the 4-fold ROV works out to $\$$`r ROV_wB` USD, and the probability of expiring in the money at the end of stage 1 increases to `r Phi2wB`.
<!-- `r round(dif_npv)` -->
