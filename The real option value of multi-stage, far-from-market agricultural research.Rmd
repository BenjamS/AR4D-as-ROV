...
In the pharmaceutical context, Cassimon et al. [-@cassimon2004valuation] extend ROV to multi-stage projects, building on the "compound option" model developed by Geske in the financial context [-@geske1979valuation]. Both Geske and Cassimon et al. derive their models from the Black-Scholes partial differentiation equation [@black1973valuation]. Here I present an alternative derivation and formulation of the multi-stage, or "$n$-fold", real option value model based on straightforward integration, which some may find more intuitive and instructive---especially if they are unfamiliar with the financial origins of Geske's model. I then apply the derived model to appraise a real 4-stage AR4D potato project. An R script of the model is included in the Appendix.
...
Since much of the target audience is probably unfamiliar with the real options literature, I build up to these results incrementally, starting with an introduction to the basic concept and elementary extension to projects with abandonment value. While AR4D is the motivating context for the present work, the $n$-fold ROV model derived here is broadly applicable to any multi-stage, far-from-market, real option context.
<!-- * R script included -->
...
It is also instructive to keep an eye on the term $\Phi(\delta)$, plotted in the lower panel of Figure \ref{fig:rovIllust}. This is the probability that the option will expire in the money, thereby triggering exercise of the option. It therefore makes perfect sense that $\Phi(\delta)$ approaches one as the moneyness is higher; but note that it approaches one more slowly as project risk is higher.
...
These upper and lower percentage errors may then be interpreted as the upper and lower bounds on the $95\%$ confidence interval about the expected percentage change in project NPV between the start and end of the project. Another phrase for "percentage change" is "arithmetic return" (call this $\mathscr{R}$), which may then be converted to a log return ($\ell = \ln(x(T) / x(\hat{t}))$) by the formula $\ell = \ln(\mathscr{R} + 1)$. Now, recall that since $x(t)$ follows a gBm with $m = r$, then $\ell$ is normally distributed with mean $\mu$ and variance $\sigma^2$ given by
...
Where $z = 1.96$ is the standard score corresponding to the $95\%$ confidence interval of a normally distributed random variable. (The standard score can of course be adjusted to match the practitioner's idea of a suitable confidence interval defined by the bounds $\bar{\ell}$ and $\underline{\ell}$.)

This system of equations can then be rearranged into three expressions for the volatility parameter $s$ in terms of known parameters (i.e. in terms of the upper and lower log returns $\bar{\ell}, \underline{\ell}$, the discount rate $r$, the project time horizon $T$, and the standard score $z$).
...
<!-- m &= \frac{1}{2 \tau} \left( \bar{\ell} + \underline{\ell} + \frac{(\bar{\ell} - \underline{\ell} )^2}{4 z^2} \right) -->
<!-- \end{split} -->
<!-- \end{equation} -->
<!-- yMax <- 1.1 -->
<!-- yMin <- -0.6 -->
...
Note also that, in the process of deducing $s$, the expected project NPV log return $\mu$ can be calculated, which may serve as a check on the plausibility of parameter values, project NPV estimates, and project NPV upper and lower bounds.
...
Before formalizing the extension of real option value to AR4D projects of four, or any number $n$, stages, note that the default cost-benefit analysis defined in expression \ref{eq:npvIneqB} can be extended to $n$-stage projects as follows.
...
Nonetheless, in such contexts, a more straightforward reason exists for setting $m = r$: By definition, project NPV appreciates over time at the quarterly discount rate $r$. Conversely put, a project growth rate different from $r$ implies that the program $NPV$ is inaccurately calculated. In the AR4D context, then, setting $m = r$ simply means assuming that project NPV is accurately estimated.
...
To see this mathematically, recall that, if $x(t)$ is a gBm, then, by Ito's lemma, the evolution of a function $f(x, t)$ is described as follows.
...
In the AR4D context, the risk free rate $r$ and no-arbitrage rule are replaced by the discount rate $r$ and the assumption that project NPV is accurately estimated, resulting in the same conclusion---that portfolio value must change at the rate of $r$. The only difference is in the meaning of $r$.
...
The groundwork has now been laid to derive the $n$-fold real option value formula. To begin, consider the far-from-market Black-Scholes PDE (equation \ref{eq:ffmBSpde}) for the 1-fold real option value function $f_1$, rearranged as an expression for $m f_1$.
...
\begin{equation}
\Delta f_1 = \left( \frac{\partial f_1}{\partial x} m x + \frac{\partial f_1}{\partial t} + \frac{x^2 s^2}{2} \frac{\partial^2 f_1}{\partial x^2} \right) \Delta t + \frac{\partial f_1}{\partial x} x s \epsilon \sqrt{\Delta t}
\end{equation}
...
By which it follows that $\Delta f_1 / f_1$ is normally distributed with mean $m \Delta t$ and variance $s^2 \eta_{1, 0}^2 \Delta t$. From this, it follows that $f_1$ is a geometric Brownian movement, such that $\ln(f_1)$ is normally distributed with mean $\ln(f_1(\hat{t})) + \left(m - s^2 \eta_{1, 0}^2 / 2 \right) \tau_1$ and variance $s^2 \eta_{1, 0}^2 \tau_1$.
...
By which it follows that $\Delta f_2 / f_2$ is normally distributed with mean $m \Delta t$ and variance $s^2 \eta_{2, 0}^2 \Delta t$; and that $\ln(f_2)$ is normally distributed with mean $\ln(f_2(\hat{t})) + \left(m - s^2 \eta_{2, 0}^2 / 2 \right) \tau_2$ and variance $s^2 \eta_{2, 0}^2 \tau_2$. That is, $f_2$ is also a geometric Brownian movement.
...
\begin{equation}
\Delta f_n = m f_n \Delta t + f_n \eta_{n, 0} s \epsilon \sqrt{\Delta t}
\label{eq:gbmfn}
\end{equation}

Such that the growth rate $\Delta f_n / f_n$ is normally distributed with the same constant mean as that of $\Delta f_0 / f_0$---i.e. $m \Delta t$---and variance $s^2 \eta_{n, 0}^2 \Delta t$, which differs from that of $\Delta f_0 / f_0$ by a factor of $\eta_{n, 0}^2$; and such that $\ln(f_n)$ is normally distributed with mean $\ln(f_n(\hat{t})) + \left(m - s^2 \eta_{n, 0}^2 / 2 \right) \tau_n$ and variance $s^2 \eta_{n, 0}^2 \tau_n$.
...
In the case of $n = 1$, it is easy to see that this reduces to the 1-fold real option value formula in equation \ref{eq:rovB}. For multi-stage projects ($n > 1$), equation \ref{eq:rovBn} must be evaluated recursively as follows:
...
* The project NPV mean growth rate $m$ and volatility $s$, or the upper and lower bounds on project NPV, whereby $m$ and $s$ can be deduced
...
No ex-ante impact assessments of LBr potato in Bangladesh and Indonesia have yet been published. For the pedagogical purposes of this exercise, say that MSU and/or USAID internally conducted an ex-ante impact assessment indicating net benefits on the order of tens of millions of dollars per year, but that the long time horizon and high discounting results in an LBr project NPV of $x(0)=\$`r x0 * 10^-6`$ million. Moreover, the study reports a high degree of uncertainty ranging from `r 100 * yMinA` $\%$ to `r 100 * yMaxA`$\%$ of NPV. Based on these uncertainty bounds (and assuming project NPV follows a gBm), the project NPV volatility parameter $s$ and quarterly growth rate $m$ are deduced following the method explained in section \ref{sec:volEst}. These work out to $m = `r m`$ (or `r round(((1 + m)^4 - 1), 2)` annually) and $s = `r s`$.

As a plausibility check, the project NPV growth rate coefficient of variation and log return are calculated based on the deduced values for $m$ and $s$. The coefficient of variation works out to `r cv`. This is high, but well within reason given the ambitious scope of the project, and given the wide range of research and non-research factors affecting its progress and eventual diffusion in the target populations and environments. The expected log return, meanwhile, works out to `r mu`, which works out to an expected arithmetic return of `r round(exp(mu) - 1, 2)`. And the expected project NPV at the end of the project works out to $\$`r ExT`$ million. These latter figures may or may not be plausible depending upon the details of the hypothetical NPV calculation. A high expected percentage change in NPV by the end of the project (i.e., a high arithmetic return) may make sense if there is reason to expect major improvements in potato value chain infrastructure within the target countries.
...
df_table <- df[, c("Stage", "OVn", "fnM10", "etaN0", "sNm1", "Phi2")]
...
df_table$ROV <- round(df_table$ROV)
df_table$`Value of\nunderlying` <- round(df_table$`Value of\nunderlying`)
these_cols <- c("Elasticity\nw.r.t. project NPV", "Standard dev.", "Phi 2")
...
df_table %>%
  knitr::kable(
    ...
    #longtable = T,
    linesep = " ",
    col.names = c(colnames(df_table)[-ncol(df_table)], "$\\Phi(\\delta_i)$"),
    escape = F
    ) %>%
  kableExtra::kable_styling(
    position = "left",
...
Based on these parameter values, the 4-fold option value of the LBr potato project works out to $\$`r as.integer(ROV)`$, which exceeds the stage 1 implementation cost ($\$`r as.integer(K1)`$) by $\$`r as.integer(dif)`$. Hence the investment is justified under the parameter settings defined above. The conventional cost-benefit analysis approach (expression \ref{eq:npvIneqBN2}), by contrast, works out to $\$`r as.integer(conv)`$ million, which falls below the stage 1 cost by $\$`r -as.integer(dif_conv)`$. On the conventional basis, then, the LBr project would be rejected.
...
The exercise so far assumes that abandonment value is zero. Now consider a stage 1 abandonment value of $\$`r as.integer(Bvec[n])`$, corresponding to the value of permanent upgrades to human and fixed capital at the national agricultural research institutions of the target countries. Then the 4-fold ROV works out to $\$`r as.integer(ROV_wB)`$ USD, and the probability of expiring in the money at the end of stage 1 increases to `r Phi2wB`.
...
The far-from-market $n$-fold ROV model derived above is, of course, not limited to the valuation of transgenic crop projects. It may be equally useful in the valuation of conventional breeding projects, including livestock projects, which also tend to unfold in a series of stages. Nor is the model limited to the AR4D context, but applies generally to the valuation of any far-from-market, multi-stage project. It can also be used in near-market, multi-stage contexts, in which case the expected project growth rate $m$ is set equal to $r$ to reflect risk-neutral valuation.

The appearance of $m$ in the far-from-market model introduces additional nuance into the ROV workflow. From a "social planner" perspective, many would say that projects with $m$ less than $r$ should not be undertaken [@moore2004just; @caplin2004social]. This makes high discount rates, such as the $0.12$ annual rate used by USAID, problematic, as it is difficult to conceive of a long term project with an NPV that grows at a rate in excess of such high benchmarks. Moreover, project ROV comes out less than project NPV when $m < r$, which does not favor the researcher. High discount rates thus create an incentive for researchers to switch to cruder, default valuation methods, or to "game" the ROV parameter setting process so as to ensure that $m > r$. This underscores the importance of a reasonable discount rate, as well as the importance of the plausibility check described in the example above.

For projects with a high expected reward (i.e. deep in the money), but low risk, there is probably no point in using the ROV approach, as the output will not differ significantly from that of conventional cost benefit analysis. The higher a project's moneyness, the higher must be its volatility for there to be a meaningful difference between the two approaches.

In some settings, it may make sense to apply the proposed $n$-fold ROV workflow at a more aggregate level of accounting. For example, the proposed method may be used to valuate an entire program, consisting of numerous projects and complementary activities at various stages of execution, over the next 20 years, broken up into four 5 year funding stages.

Project NPV is often a function of the scale of launch etc. That is, project NPV can be increased or decreased by increasing or decreasing the scope--by adding or removing countries where the new tech is to be released, and this in turn is achieved by increasing or decreasing the funds allocated to launch and uptake. decreasing returns $x_0 = K^\alpha ; \:\:\: 0<\alpha<1$...

<!-- # Conclusion -->
<!-- by removing, in some measure, the methodological basis for the toxicity. -->
<!-- can 1) provide meaningful, risk-adjusted project appraisals, 2) secure long term commitments from donors, while at the same time 3) giving donors the flexibility they require to maintain solvent fiscal outlooks. -->
...
rovN <- function(X0, Kvec, Bvec = NULL, Tvec, s, m, r){
  ...
  for(i in 1:N){
    ...
    Bn <- Bvec[i]
    Tn <- rev(cumsum(rev(Tvec)))[i]
    dn <- (log(fNm1 / (Kn - Bn)) + (m - s^2 * etaNm10^2 / 2) *
    Tn) / (s * etaNm10 * sqrt(Tn))
    Phi1 <- pnorm(dn + s * etaNm10 * sqrt(Tn))
    Phi2 <- pnorm(dn)
    fn <- exp((m - r) * Tn) * fNm1 * Phi1 - exp(-r * Tn) *
    Kn * Phi2 + exp(-r * Tn) * Bn * (1 + Phi2)
    fnVec[i] <- fn
    sNm1vec[i] <- s * etaNm10
    Phi2vec[i] <- Phi2
    Phi1vec[i] <- Phi1
    phi1 <- dnorm(dn + s * etaNm10 * sqrt(Tn))
    dfndfNm1 <- exp((m - r) * Tn) * (Phi1 + phi1 * 
    (1 - etaNm10) * s * sqrt(Tn))
    etaNnM1 <- fNm1 / fn * dfndfNm1
    etaNm10 <- etaNnM1 * etaNm10
    fNm1 <- fn
