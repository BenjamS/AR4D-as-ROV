...
In the ROV literature, $m$ is usually set equal to $r$ due to an artifact inherited from the financial context called "risk neutral valuation". Farther below, I demonstrate that there is no theoretical premise for the assumption of risk neutral valuation in the AR4D context. Nonetheless, $m$ may be set equal to $r$ on the basis that project NPV must, by definition, be expected to appreciate at the rate of $r$. Setting $m$ to a value other than $r$ implies that project NPV is inaccurately estimated. Assuming project NPV is accurately estimated, then, it must be that $m = r$, and the $ROV$ expression simplifies to
...
Nonetheless, a straightforward reason exists for setting $m = r$ in far-from-market settings: By definition, NPV must be expected to appreciate at the discount rate $r$. Conversely put, an expected NPV growth rate different from $r$ implies that the program $NPV$ is inaccurately estimated. In the AR4D context, then, setting $m = r$ simply implies the assumption that project NPV is accurately estimated.
...
In the AR4D context, the risk free rate and no-arbitrage rule are replaced by the discount rate (still denoted by $r$) and the assumption that project NPV is accurately estimated, resulting in the same conclusion---that portfolio value must change at the rate of $r$. The only difference is in the meaning of $r$.
...
With the groundwork laid above, it is now time to derive the $n$-fold real option value formula. To begin, consider the far-from-market Black-Scholes PDE (equation \ref{eq:ffmBSpde}) for the 1-fold real option value function $f_1$, rearranged as an expression for $r f_1$.
...
\begin{equation}
re f_1 = \frac{\partial f_1}{\partial x} x r + \frac{\partial f_1}{\partial t} + \frac{s^2 x^2}{2} \frac{\partial^2 f_1}{\partial x^2}
\end{equation}

And consider Ito's lemma for $f_1$.

\begin{equation}
\Delta f_1 = \left( \frac{\partial f_1}{\partial x} r x + \frac{\partial f_1}{\partial t} + \frac{x^2 s^2}{2} \frac{\partial^2 f_1}{\partial x^2} \right) \Delta t + \frac{\partial f_1}{\partial x} x s \epsilon \sqrt{\Delta t}
\end{equation}
...
By which it follows that $\Delta f_1 / f_1$ is normally distributed with mean $r \Delta t$ and variance $s^2 \eta_{1, 0}^2 \Delta t$. From this, it follows that $f_1$ is a geometric Brownian movement, such that $\ln(f_1)$ is normally distributed with mean $\ln(f_1(\hat{t})) + \left(r - s^2 \eta_{1, 0}^2 / 2 \right) \tau_1$ and variance $s^2 \eta_{1, 0}^2 \tau_1$.

It follows that a 2-fold real option value function $f_2(f_1, t)$ also has a far-from-market Black-Scholes PDE.

\begin{equation}
\left( f_2 - \frac{\partial f_2}{\partial f_1} f_1 \right) r = \frac{\partial f_2}{\partial t} + \frac{1}{2} \frac{\partial^2 f_2}{\partial f_1^2} s^2 f_1^2
\end{equation}

With corresponding Ito's lemma

\begin{equation}
\Delta f_2 = \left( \frac{\partial f_2}{\partial f_1} r f_1 + \frac{\partial f_2}{\partial t} + \frac{1}{2} \frac{\partial^2 f_2}{\partial f_1^2} s^2 f_1^2 \right) \Delta t + \frac{\partial f_2}{\partial f_1} f_1 s \epsilon \sqrt{\Delta t}
\end{equation}
...
\begin{equation}
\begin{split}
\Delta f_2 &= r f_2 \Delta t + f_2 \eta_{1, 0}  \frac{\partial f_2}{\partial f_1} s \epsilon \sqrt{\Delta t} \\
&= r f_2 \Delta t + f_2 \eta_{2, 0} s \epsilon \sqrt{\Delta t}
\end{split}
\label{eq:gbmf2}
\end{equation}

By which it follows that $\Delta f_2 / f_2$ is normally distributed with mean $m \Delta t$ and variance $s^2 \eta_{2, 0}^2 \Delta t$; and that $\ln(f_2)$ is normally distributed with mean $\ln(f_2(\hat{t})) + \left(r - s^2 \eta_{2, 0}^2 / 2 \right) \tau_2$ and variance $s^2 \eta_{2, 0}^2 \tau_2$. That is, $f_2$ is also a geometric Brownian movement.

And so on for $f_3$, $f_4$, $\dots$, $f_n$, there exists a far-from-market Black-Scholes PDE

\begin{equation}
\left( f_i - \frac{\partial f_i}{\partial f_{i - 1}} f_{i - 1} \right) r = \frac{\partial f_i}{\partial t} + \frac{1}{2} \frac{\partial^2 f_i}{\partial f_{i - 1}^2} s^2 f_{i - 1}^2 \; & i \in (1, n)
\end{equation}

Which, when combined with the corresponding Ito's lemma, results in an expression for $\Delta f_i$ as a geometric Brownian movement.

\begin{equation}
\Delta f_i = r f_i \Delta t + f_i \eta_{i, 0} s \epsilon \sqrt{\Delta t}
\label{eq:gbmfn}
\end{equation}

Such that the growth rate $\Delta f_i / f_i$ is normally distributed with the same constant mean as that of $\Delta f_0 / f_0$---i.e. $m \Delta t$---and variance $s^2 \eta_{i, 0}^2 \Delta t$, which differs from that of $\Delta f_0 / f_0$ by a factor of $\eta_{i, 0}^2$; and such that $\ln(f_i)$ is normally distributed with mean $\ln(f_i(\hat{t})) + \left(r - s^2 \eta_{i, 0}^2 / 2 \right) \tau_i$ and variance $s^2 \eta_{i, 0}^2 \tau_i$.
...
ROV_wB <- round(df_wB$OVn[nrow(df_wB)])
dif_wB <- round(ROV_wB - K1)
Phi2wB <- round(df_wB$Phi2[n], 2)
etaN0_wB <- round(df_wB$etaN0[n], 2)
#--------------------------------------------------------------------------
...
In this article, I have developed an $n$-fold ROV model to evaluate the real option value of $n$-stage AR4D projects. The model is useful compared to the default CBA approach because... accounts for risk.... Options to abandon are built in,   facilitates donor commitment to the long time horizons of agricultural research.

It is important to keep in mind that the ROV of projects with a high expected reward (i.e. that are deep in the money), but low risk, will not differ significantly from their NPV. The higher a project's moneyness, the higher must be its volatility for there to be a meaningful difference between the ROV and conventional CBA approaches. In the case of LBr potato examined above, for example, a small increase in the hypothetical NPV and/or decrease in the confidence interval given by $\bar{\ell}$, $\underline{\ell}$ (and hence a decrease in the volatility $s$) results in an ROV that is virtually the same as the NPV, in which case there is no point in evaluating the ROV.
...
Project NPV usually scales with the launch or "scale up" cost $K_n$. That is to say, beyond a certain minimum level of funding required to generate, release, and support the diffusion and uptake of the project's research product on a pilot level, further increments in project investment have the effect of broadening the scope of impact---i.e. extending the size and number of target populations and environments where the research product is released and has an impact. In future work, it may thus be worthwhile to explore the ROV implications of defining project NPV as a function of the launch investment $K_n$. For example, $x(0)$ could be defined

\begin{equation}
x(0) = \tilde{x}(0) \left(\frac{K_n}{\bar{K}_n}\right)^{\alpha}
\end{equation}

Where the parameter $\bar{K}_n$ is a reference value---perhaps the minimum or maximum possible scale up investment, for example---$\tilde{x}(0)$ is the project NPV if launch funding equals the reference value, and the exponent $\alpha$ is restricted to fall between 0 and 1, reflecting decreasing returns to scale. It may then be interesting to plot ROV against successively higher scale up investments $K_n$, corresponding, for example, to successively broader geographical scopes of impact.

For pedagogical purposes, here I focus on transgenic projects; but the model may be applied to the evaluation of conventional crop and livestock breeding projects as well.The far-from-market $n$-fold ROV model derived in this article is, of course, not limited to the valuation of transgenic crop projects. It may be equally useful in the valuation of conventional breeding projects, including livestock projects; but the precise staging of such projects can vary considerably, and so must be assessed on a case by case basis. Nor is the model limited to the AR4D context, but applies generally to the valuation of any far-from-market, multi-stage project.
<!-- It can also be used in near-market, multi-stage contexts, in which case the expected project growth rate $m$ is set equal to $r$ to reflect risk-neutral valuation. -->
