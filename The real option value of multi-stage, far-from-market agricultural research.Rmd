...
In the AR4D context, the risk free rate and no-arbitrage argument are replaced by the discount rate (also denoted by $r$) and the assumption that project NPV is accurately estimated, resulting in the same conclusion---that the value of the riskless portfolio on the left-hand side of equation \ref{eq:bsInsight1} must change at the rate of $r$.
...
\begin{equation}
\left( f - \frac{\partial f}{\partial x} x \right) r = \frac{\partial f}{\partial t} + \frac{1}{2} \frac{\partial^2 f}{\partial x^2} x^2 s^2
\end{equation}

(Black and Scholes went on to solve this PDE under the boundary condition $f(T) = \max(x(T) - K, 0)$, resulting in an expression for $f$ identical to the one derived in equation \ref{eq:EmaxTK}, with the exception that $m$ is replaced with $r$.)

Now, letting $f = f_1$ and rearranging the Black-Scholes PDE as an expression for $r f_1$,

\begin{equation}
r f_1 = \frac{\partial f_1}{\partial x} x r + \frac{\partial f_1}{\partial t} + \frac{s^2 x^2}{2} \frac{\partial^2 f_1}{\partial x^2}
\end{equation}
...
By which it follows that $\Delta f_2 / f_2$ is normally distributed with mean $m \Delta t$ and variance $s^2 \eta_{2, 0}^2 \Delta t$; and that $\ln(f_2)$ is normally distributed with mean $\ln(f_2(\hat{t})) + \left(r - s^2 \eta_{2, 0}^2 / 2 \right) \tau_2$ and variance $s^2 \eta_{2, 0}^2 \tau_2$. That is, $f_2$ is also a gBm.
...
Which, when combined with the corresponding Ito's lemma, results in an expression for $\Delta f_i$ as a gBm.
...
Such that the growth rate $\Delta f_i / f_i$ is normally distributed with the same constant mean as that of $\Delta f_0 / f_0$---i.e. $r \Delta t$---and variance $s^2 \eta_{i, 0}^2 \Delta t$, which differs from that of $\Delta f_0 / f_0$ by a factor of $\eta_{i, 0}^2$; and such that $\ln(f_i)$ is normally distributed with mean $\ln(f_i(\hat{t})) + \left(r - s^2 \eta_{i, 0}^2 / 2 \right) \tau_i$ and variance $s^2 \eta_{i, 0}^2 \tau_i$.
...
# Results: An illustrative example of real option valuation of an AR4D project
