...
Even in the corporate ROV context, the replicating portfolios argument is met with considerable skepticism [@borison2005real]; and proponents of the argument acknowledge that its validity is limited to a narrow range of project types---which generally does not include R&D projects [@schwartz2013real]. There is nonetheless strong methodological pressure to invoke the argument because the fundamental equation governing financial option (and all derivative) valuation---i.e., the Black-Scholes partial differential equation (PDE)---which one wishes to adapt to real project settings, is formally derived from an assumption in financial markets known as the "no-arbitrage rule", which requires that the underlying project NPV be replicable as a traded good in an efficient market [@hull9thEdition]. In cases where the replicating portfolios argument may not be invoked, i.e. where project NPV may not be replicable as a traded good, some corporate ROV practitioners invoke the "complete markets" assumption as a second-best alternative [majd; pennings]. Under the complete markets assumption, project NPV cannot be modeled as a linear function of prices, but the Black-Scholes PDE is nonetheless preserved because "all risks can be hedged by trading securities" [pennings].

As mentioned in the previous paragraph, the main source of AR4D project risk is technical risk associated with non-market factors. It follows that AR4D risk cannot be hedged by trading securities; and thus there are no grounds on which to assume complete markets in the AR4D context.
